 ...
 */
Self.getASAPAuthorByName = function (name) {
  return Self.ASAPAuthorsList.data[Self.ASAPAuthorsList.mappings.name[name]];
};
...
Self.buildAuthorsList = function (data) {
  ...
  for (let i = 0; i < Self.ASAPAuthorsList.data.length; i++) {
    ...
    Self.ASAPAuthorsList.raw.push(name);
    Self.ASAPAuthorsList.mappings.name[name] = i;
 ...
 * @returns {array} List of authors
 */
Self.refreshASAPAuthors = function (cb) {
...
Self.findAuthorFromASAPList = function (name) {
  ...
  let matches = stringSimilarity.findBestMatch(name, Self.ASAPAuthorsList.raw);
  ...
  return [].concat(
    matches.ratings
      ...
      .map((item) => {
        return Object.assign({}, Self.getASAPAuthorByName(item.target), { rating: item.rating });
      })
  );
};
