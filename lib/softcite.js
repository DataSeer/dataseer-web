...
const service = require(`../conf/services/softcite.json`);

let Self = {};

/**
 * Send sentence to softcite service
...
const sendFile = function (opts = {}, url, cb) {
  ...
  return fetch(url, {
    ...
    .then(function (res) {
      // res.status >= 200 && res.status < 300
      if (res.ok) return res;
      else throw new Error(`Softcite service is not responding (${res.status} ${res.statusText})`);
    })
    .then(function (res) {
      return res.buffer();
    })
