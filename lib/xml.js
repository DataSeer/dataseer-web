...
const PDF_METADATA_VERSION = 2;
const XML_METADATA_VERSION = 2;
...
Self.addDataset = function ($, opts) {
  ...
  if (encodingDesc.length === 1) {
    if (sentence.length === 1) {
      ...
      let dataset = $('<dataset>'),
        ...
        _lastDatasetId = datasetsList
          ...
          .map(function (index, element) {
            return parseInt($(element).attr('id').replace('dataset-', ''), 10);
          })
          .get()
          .sort(function (a, b) {
            if (a < b) return -1;
        ...
        dataInstanceNumber = lastDataInstanceId ? lastDataInstanceId + 1 : 0,
        dataInstanceId =
      ...
      dataset.attr(`id`, datasetId).attr('type', opts.dataset.type).attr('subtype', opts.dataset.subtype);
...
Self.updateDataset = function ($, opts) {
  let data = { dataset: {} },
    err = true,
    encodingDesc = $(`teiHeader encodingDesc`),
    dataset = encodingDesc.find(`list[type="dataset"] > dataset[id="${opts.dataset.id}"]`),
  ...
  if (dataset.length === 1) {
    if (dataInstance.length === 1) {
      ...
      data.dataset.id = dataset.attr('id');
      data.dataset.dataInstanceId = dataInstance.attr('id');
...
Self.addSentencesId = function (xmlStr) {
  ...
  if ($) {
    $('TEI s[coords]').map(function (i, el) {
      ...
      // $(el).attr(`xml:id`, `sentence-${i}`); // when dataseer-ml will be updated
    });
    return $.xml();
  } else return xmlStr;
};
