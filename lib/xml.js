...
const xmlConf = require(`../conf/xml.json`);
const DASConf = require(`../conf/DAS.json`);
const MASConf = require(`../conf/MAS.json`);
...
Self.selectors = {
  metadata: {
    ...
    DAS: function ($) {
      let headDASSentences = $(`head > s[xml\\:id]`)
        .get()
        .filter(function (item) {
          let elem = $(item);
          return DASConf.indexOf(elem.text().toLowerCase()) > -1;
        });
      if (headDASSentences.length <= 0) return { content: ``, sentences: [] };
      let sentences = $(headDASSentences[headDASSentences.length - 1])
        .parent()
        .parent()
        .find(`s[xml\\:id]`)
        .get()
    ...
    MAS: function ($) {
      let headMASSentences = $(`head > s[xml\\:id]`)
        .get()
        .filter(function (item) {
          let elem = $(item);
          return MASConf.indexOf(elem.text().toLowerCase()) > -1;
        });
      if (headMASSentences.length <= 0) return { content: ``, sentences: [] };
      let sentences = $(headMASSentences[headMASSentences.length - 1])
        .parent()
        .parent()
        .find(`s[xml\\:id]`)
        .get()
 ...
 * @return {string} XML string
 */
Self.addSentences = function (xmlString, sentences, type = `custom`) {
  let $ = Self.load(xmlString);
  let body = $(`text body`);
  let sentencesListElement = body.find(`div[type="${type}-sentences"]`);
  if (!sentencesListElement.length) {
    sentencesListElement = $(`<div>`).attr(`type`, `${type}-sentences`);
    body.append(sentencesListElement);
  }
  for (let i = 0; i < sentences.length; i++) {
    let sentence = sentences[i];
    let sentenceElement = $(`<s>`)
      .attr(`xml:id`, sentence.id ? sentence.id : `${type}-sentence-${process.hrtime.bigint()}`)
...
/**
  * Get all head sentences of given sentences
  * @param {object} $ - XML parsed cheerio Object
  * @param {object} opts - JSON containing all data
  * @param {string} opts.sentences - All sentences
 ...
 */
Self.getHeadSentences = function ($, opts) {
  let results = [];
  let ids = opts.sentences.map(function (s) {
    return s.id;
  });
  for (let i = 0; i < ids.length; i++) {
    let sentenceId = ids[i];
    if (sentenceId.indexOf(`krt-sentence`) > -1) continue;
    if (sentenceId.indexOf(`ocr-sentence`) > -1) continue;
    let sentence = $(`s[xml\\:id="${sentenceId}"]`);
    if (sentence.length === 1) {
      let parent = sentence.parent();
      let greatParent = parent.parent();
      let headSentences = greatParent
        ? greatParent
          .find(`head > s`)
          ...
          .get()
        : [];
      if (Array.isArray(headSentences) && headSentences.length > 0) results = results.concat(headSentences);
    }
  }
  return results;
};
...
Self.extractTEISentences = function ($, type = `array`) {
  ...
  $(`s[xml\\:id]`).map(function (i, el) {
    let element = $(el);
    let isHead = element.parent(`head`).length;
    if (Array.isArray(sentences)) sentences.push({ id: element.attr(`xml:id`), text: element.text(), head: isHead });
    else
      sentences[element.attr(`xml:id`)] = {
...
/**
 * Check the content of a header sentence
 * @param {string} text - Text of the header sentence
 * @returns {boolean} True if the content match, else false
 */
Self.checkHeaderContent = function (text) {
  let tests = xmlConf.headers;
  let txt = text.toLowerCase().replace(/[\n\s]+/gm, ` `);
  let result = false;
  let i = 0;
  while (!result && i < tests.length - 1) {
    let test = tests[i].toLowerCase();
    result = test === txt || test.split(txt).length > 1 || txt.split(test).length > 1;
    i++;
  }
  return result;
};
