    ...
    'datasets': function($) {
      ...
      $('div[subtype=dataseer] s[id]')
        .map(function(i, el) {
          ...
          let id = $(el).attr('id'),
            type = $(el).attr('type'),
            cert = $(el).attr('cert') ? $(el).attr('cert') : '0',
...
object.buildDataTypes = function(data) {
  ...
  const extractDataTypes = function(key, obj, result, parent, deep = 0) {
    if (key === 'Tabular Data') key = 'Tabular data';
    let properties = { 'mesh_id': true, 'url': true, 'description': true, 'count': true },
    ...
    } else if (result.dataTypes[key] === 'undefined') result.dataTypes[key] = [];
