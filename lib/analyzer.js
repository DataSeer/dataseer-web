...
Self.checkDataObjectsInTEI = function (opts, cb) {
  ...
  let sentences = _.flatten(
    opts.dataObjects.map(function (dataObject) {
      return dataObject.sentences.map(function (sentence) {
        return {
  ...
  for (let i = 0; i < sentences.length; i++) {
    ...
    let dataObject = sentences[i].dataObject;
    if (typeof mapping[dataObject._id] === `undefined`)
      mapping[dataObject._id] = { dataObject: dataObject, searches: [] };
    for (let j = 0; j < paragraphs.length; j++) {
      let search = Self.search(sentence.text, paragraphs[j]);
      if (search.matches.length > 0)
        mapping[dataObject._id].searches.push({
          id: sentence.id,
          text: sentence.text,
  ...
  for (let id in mapping) {
    ...
    let dataObject = JSON.parse(JSON.stringify(mapping[id].dataObject));
    ...
    if (searches.length > 0) {
      ...
      if (dataObject.sentences.length > 0) dataObjects.mergeable.push(dataObject);
      else dataObjects.rejected.push(mapping[id].dataObject);
    } else dataObjects.rejected.push(mapping[id].dataObject);
  }
