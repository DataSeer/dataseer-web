...
Self.create = function (opts = {}, cb) {
  ...
  return fs.readFile(opts.path, `binary`, function (err, content) {
...
Self.getData = function (doc, dataTypes) {
  ...
  let orderedDatasets = doc.datasets.current
    ...
    .map(function (item, i) {
      let infos = DataTypes.getDataTypeInfos(item, dataTypes, { convertHTML: true });
      return {
        reuse: item.reuse,
  ...
  let code = orderedDatasets.filter(function (item) {
  ...
  return {
    ...
    suggestedDataAvailabilityStatements: protocols.concat(code, reagents, datasets),
    bestPractices: DataTypes.getBestPractices(protocols.concat(code, reagents, datasets), dataTypes, {
      convertHTML: true
    }).data,
    protocols: protocols,
    code: code,
