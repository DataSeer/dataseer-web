  ...
  let orderedDatasets = doc.datasets.current
    ...
    .map(function (item, i) {
      return {
        ...
        type: DocumentsDatasetsController.getDataTypeInfos(item, dataTypes).label,
        text: item.sentences
          .sort(sortSentences)
          .map(function (item) {
            return item.text;
        ...
        suggestedDataAvailabilityStatement: item.DOI
          ...
          : { isLink: 'false', url: '', label: 'N/A' },
        description: DocumentsDatasetsController.getTextOfHtml(item.description),
        bestDataFormatForSharing: item.reuse
          ? item.bestPracticeForIndicatingReUseOfExistingData
            ? DocumentsDatasetsController.getTextOfHtml(item.bestPracticeForIndicatingReUseOfExistingData)
            : DocumentsDatasetsController.getTextOfHtml('<p>N/A</p>')
          : item.bestDataFormatForSharing
          ? DocumentsDatasetsController.getTextOfHtml(item.bestDataFormatForSharing)
          : DocumentsDatasetsController.getTextOfHtml('<p>N/A</p>'),
        mostSuitableRepositories: item.reuse
          ? item.mostSuitableRepositories.default
            ? DocumentsDatasetsController.getTextOfHtml(item.mostSuitableRepositories.default)
            : DocumentsDatasetsController.getTextOfHtml('<p>N/A</p>')
          : item.mostSuitableRepositories.reuse
          ? DocumentsDatasetsController.getTextOfHtml(item.mostSuitableRepositories.reuse)
          : DocumentsDatasetsController.getTextOfHtml('<p>N/A</p>')
      };
    });
