...
Self.extractDataFromSource = function (source, $) {
  ...
  case `searchResources`:
    return $(`.inner-results`)
      .map((i, e) => {
        ...
        let suggestedName = split[0].trim();
        let suggestedURL = element.children(`a`).first().attr(`href`);
        let suggestedRRID = `${suggestedName} (RRID:${RRID})`;
        return { RRID, suggestedName, suggestedURL, suggestedRRID };
      })
      .get();
    break;
  case `searchAntibody`:
    return $(`.inner-results`)
      .map((i, e) => {
        let element = $(e);
        let RRID = element.find(`.the-title > ul > h4`).text().replace(/\s+/gm, ` `).trim();
        let suggestedName = element.find(`.the-title > ul > h3 > div > a`).text().replace(/\s+/gm, ` `).trim();
  ...
  case `searchTool`:
    return $(`.inner-results`)
      .map((i, e) => {
        let element = $(e);
        let RRID = element.find(`.the-title > .list-inline > h4`).text().replace(/\s+/gm, ` `).trim();
        let suggestedName = element
