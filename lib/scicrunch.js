  ...
  case `searchResources`:
    return $(`.inner-results`)
      .map((i, e) => {
        ...
        let suggestedRRID = element
          .find(`p:nth-of-type(4)`)
          .text()
          .replace(/\s+/gm, ` `)
          .replace(`Proper citation: `, ``)
          .replace(`Copy`, ``)
          .trim();
...
Self.searchEntity = function (opts, cb) {
  ...
  if (typeof _.get(opts, `kind`) === `undefined`) return cb(null, new Error(`Missing required data: opts.kind`));
  // Build URLs
  ...
  let URLs = {};
  if (opts.kind === `software` || opts.kind === `code`) {
    URLs.searchTool = URLMANAGER.build(service.searchTool, { q: opts.entity, l: opts.entity }, service.root);
  } else if (opts.kind === `reagent`) {
    URLs.searchAntibody = URLMANAGER.build(service.searchAntibody, { q: opts.entity, l: opts.entity }, service.root);
    URLs.searchPlasmid = URLMANAGER.build(service.searchPlasmid, { q: opts.entity, l: opts.entity }, service.root);
    URLs.searchOrganism = URLMANAGER.build(service.searchOrganism, { q: opts.entity, l: opts.entity }, service.root);
    URLs.searchCellLine = URLMANAGER.build(service.searchCellLine, { q: opts.entity, l: opts.entity }, service.root);
  } else return cb(null, new Error(`searchEntity functionality not available for this kind of data object`));
  let results = {};
  return async.map(
    Object.keys(URLs),
    function (key, next) {
    ...
    .then(function (body) {
      ...
      if (Array.isArray(body.data) && body.data.length > 0) {
        for (let i = 0; i < body.data.length; i++) {
          RRIDsMap[body.data[i].scr_id] = body.data[i];
        }
      }
