...
const jsonCustomList = require(`../resources/jsonCustomReagentsList.json`);
 ...
 * @returns {array} List of items
 */
Self.refreshCustomList = function (opts = {}, cb) {
  if (typeof opts !== `object`) opts = {};
  if (typeof opts.local === `undefined`) opts.local = false;
  if (typeof process.env.LOCAL_CUSTOM_REAGENTS_LIST !== `undefined` || opts.local)
    return cb(null, Self.buildCustomList(jsonCustomList));
  return GoogleSheets.getCustomReagents(function (err, data) {
 ...
 */
Self.findFromCustomList = function (name) {
  ...
  if (matches.bestMatch.rating <= conf.settings.findFromCustomList.minRating) return notFoundError;
  return [].concat(
    matches.ratings
      .map((item, i) => {
        return item.rating >= conf.settings.findFromCustomList.minRating
