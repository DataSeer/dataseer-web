...
/**
 * This function return the save file ID (google file ID)
 ...
 */
Self.getSaveFileId = function (opts = {}, cb) {
  ...
  return async.doWhilst(
    ...
    function (err) {
      ...
      if (ids.length <= 0) return cb(null, new Error(`Save file not found`));
...
/**
 * This function will create the save file and return the file ID (google file ID)
 * @param {object} opts - Options available
 * @param {object} opts.data - Data available
 * @param {string} opts.data.name - Name of the document
 ...
 * @returns {undefined} undefined
 */
Self.createSaveFile = function (opts = {}, cb) {
  // Check all required data
  ...
  return Self.getSaveFileId(_opts, function (err, res) {
    if (err) return cb(err);
    // Case report does not exist yet
    if (res instanceof Error)
      return Self._createSaveFile(_opts, function (err, res) {
        if (err) return cb(err);
        return cb(err, res);
      });
    ...
    return Self._deleteSaveFile({ folder: opts.folder, data: { fileId: res } }, function (err, res) {
      if (err) return cb(err);
      return Self._createSaveFile(_opts, function (err, res) {
        if (err) return cb(err);
        return cb(err, res);
...
/**
 * This function will create the save file and return the save file ID (google file ID)
 * @param {object} opts - Options available
 ...
 * @returns {undefined} undefined
 */
Self._createSaveFile = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(conf, `owner`) === `undefined`) return cb(Error(`Missing required data: conf.owner`));
 ...
 * @returns {undefined} undefined
 */
Self.deleteSaveFile = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `folder`) === `undefined`) return cb(Error(`Missing required data: opts.folder`));
  if (typeof _.get(opts, `data.name`) === `undefined`) return cb(Error(`Missing required data: opts.data.name`));
  let ignore = _.get(opts, `ignore`, false);
  return Self.getSaveFileId({ folder: opts.folder, data: { name: opts.data.name } }, function (err, res) {
    if (err) return cb(err);
    ...
    // Erase file
    return Self._deleteSaveFile({ data: { fileId: res } }, function (err, res) {
      if (err) return cb(err);
      return cb(err, true);
    });
  });
 ...
 * @returns {undefined} undefined
 */
Self._deleteSaveFile = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `folder`) === `undefined`) return cb(Error(`Missing required data: opts.folder`));
