 ...
 * @returns {undefined} undefined
 */
Self.buildChangesFromDSLogs = function (opts, cb) {
  // Check all required data
  let kind = _.get(opts, `kind`);
  ...
  return Self.createSheetsFile(
    ...
    function (err, id) {
      ...
      if (kind === `REPORTS_CHANGES`)
        return Self.insertDataInChangesFromReportsLogsFile({ spreadsheetId: id, data: opts.data }, function (err) {
          if (err) return cb(err);
          return cb(null, id);
        });
      else if (kind === `DS_LOGS`)
...
Self.fillResearchOutputAvailabilityStatementAndOtherInformationASAPPart = function (opts = {}, cb) {
  ...
  let actions = [
    ...
    function (next) {
      ...
      researchOutputAvailabilityStatement.push(
        Self.buildRequestUpdateBorders(sheetId, 41, 41 + lines.all.length - 1, 6, 9, GRAY_BORDERS)
      );
      researchOutputAvailabilityStatement.push(
      ...
      for (let i = 0; i < lines.all.length; i++) {
        ...
        data.push(
          Self.buildRequestUpdateCell(`'Summary Sheet'!I${rowIndex}`, [
            `=OR(IF(IFERROR(VLOOKUP(H${rowIndex}, 'ASAP Identifiers'!B:B, 1, FAUX), FAUX) = FAUX, FAUX, VRAI), SI(IFERROR(VLOOKUP(H${rowIndex}, 'ASAP Identifiers'!C:C, 1, FAUX), FAUX) = FAUX, FAUX, VRAI), SI(IFERROR(VLOOKUP(H${rowIndex}, 'ASAP Identifiers'!D:D, 1, FAUX), FAUX) = FAUX, FAUX, VRAI), SI(IFERROR(VLOOKUP(H${rowIndex}, 'ASAP Identifiers'!E:E, 1, FAUX), FAUX) = FAUX, FAUX, VRAI), SI(IFERROR(VLOOKUP(H${rowIndex}, 'ASAP Identifiers'!F:F, 1, FAUX), FAUX) = FAUX, FAUX, VRAI))`
          ])
        );
      }
