...
Self.buildReport = function (opts = {}, cb) {
  ...
  for (let i = 0; i < opts.data.doc.organizations.length; i++) {
    let organizationId = opts.data.doc.organizations[i];
    if (typeof folders[organizationId] !== `undefined`) {
      folder = folders[organizationId];
      break;
    }
  ...
  return Self.createReportFile(
    ...
    function (err, id) {
      ...
      if (kind === `ASAP-PPMI`)
        return Self.insertDataInASAPPPMISheets({ spreadsheetId: id, data: opts.data }, function (err) {
          if (err) return cb(err);
          return cb(null, id);
        });
      if (kind === `ASAP-GP2`)
        return Self.insertDataInASAPGP2Sheets({ spreadsheetId: id, data: opts.data }, function (err) {
          if (err) return cb(err);
          return cb(null, id);
        });
        ...
        return Self.insertDataInUniversalJournalReportSheets({ spreadsheetId: id, data: opts.data }, function (err) {
          if (err) return cb(err);
          return cb(null, id);
        });
      ...
      data: [
        {
          ...
          data: softwares.map(function (item) {
            return {
              reuse: item.reuse,
              name: conf.settings.datasetsLinks[`Universal Journal Report`]
                ? Self.buildDatasetLink(documentId, documentToken, item.id, item.name)
                : item.name,
      ...
      data: [
        {
          ...
          data: softwares.map(function (item) {
            return {
              reuse: item.reuse,
              name: conf.settings.datasetsLinks[`AmNat`]
                ? Self.buildDatasetLink(documentId, documentToken, item.id, item.name)
                : item.name,
              sentences: item.sentences
                .map(function (s) {
...
Self.insertDataInDataSeerGenericSheets = function (opts = {}, cb) {
  ...
  let filteredDatasets = [
    ...
    datasets.filter(function (item) {
      return item.status === RULES[`default`].status.datasets[`4`].label;
    }),
    // Cite dataset unique identifier in text
    datasets.filter(function (item) {
      return item.status === RULES[`default`].status.datasets[`5`].label;
    }),
    // Verify dataset citation
    datasets.filter(function (item) {
      return item.status === RULES[`default`].status.datasets[`3`].label;
    }),
    // Done - Cited correctly
    datasets.filter(function (item) {
      return item.status === RULES[`default`].status.datasets[`0`].label;
    }),
    // Not required, but recommended to include the representative image/video files in raw format with dataset upload.
    datasets.filter(function (item) {
      return item.status === RULES[`default`].status.datasets[`1`].label;
    }),
    // Confirmatory and Quality Control datasets are not required to be included in your dataset upload.
    datasets.filter(function (item) {
      return item.status === RULES[`default`].status.datasets[`2`].label;
    })
  ];
  let filteredCodes = [
    ...
    codes.filter(function (item) {
      ...
      return item.status === RULES[`default`].status.codes[subType][`2`].label;
    }),
    // Verify citation
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`default`].status.codes[subType][`1`].label;
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`default`].status.codes[subType][`0`].label;
    })
  ];
  let filteredSoftwares = [
    ...
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`default`].status.softwares[subType][`2`].label;
    }),
    // Verify citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`default`].status.softwares[subType][`1`].label;
    }),
    // None - Citation meets minimum standards
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`default`].status.softwares[subType][`0`].label;
    })
  ];
  let filteredMaterials = [
    // Assign RRID and cite in text
    reagents.filter(function (item) {
      return item.status === RULES[`default`].status.reagents[`3`].label;
    }),
    // Check for an existing RRID
    reagents.filter(function (item) {
      return item.status === RULES[`default`].status.reagents[`2`].label;
    }),
    // Verify citation
    reagents.filter(function (item) {
      return item.status === RULES[`default`].status.reagents[`1`].label;
    }),
    // Done - Cited correctly
    reagents.filter(function (item) {
      return item.status === RULES[`default`].status.reagents[`0`].label;
    })
  ];
  let filteredProtocols = [
    ...
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`default`].status.protocols[subType][`2`].label;
    }),
    // Include missing information in citation
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`default`].status.protocols[subType][`3`].label;
    }),
    // Verify citation
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`default`].status.protocols[subType][`1`].label;
    }),
    // Done - Cited correctly
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`default`].status.protocols[subType][`0`].label;
    })
  ];
        ...
        return Self.fillResearchOutputAvailabilityStatementDataSeerGenericPart(
          {
            ...
            data: {
              ...
              codes: filteredDataObject.codes.filtered[2].filter(function (item) {
                return !item.reuse;
              }),
              softwares: filteredDataObject.softwares.filtered[2].filter(function (item) {
                return !item.reuse;
              }),
...
Self.fillComplianceReportDataSeerGenericPart = function (opts = {}, cb) {
  ...
  let batches = [
      ...
      data: [
        {
          ...
          row: { min: 127, max: 132, insert: 130 },
          cells: [
            { column: `B`, property: `reuse` },
          ...
          data: protocols[0].map(function (item) {
            return {
              reuse: item.reuse,
              issue: !!item.issues,
              name: conf.settings.datasetsLinks[`DataSeer Generic`]
                ? Self.buildDatasetLink(documentId, documentToken, item.id, item.name)
                : item.name,
              notes: item.comments,
              sentences: item.sentences
        ...
        },
        {
          ...
          row: { min: 133, max: 138, insert: 136 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
        ...
        },
        {
          ...
          row: { min: 139, max: 144, insert: 142 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
        ...
        },
        {
          ...
          row: { min: 145, max: 150, insert: 147 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
    ...
    // lab materials
    {
      row: { min: 95, max: 95, insert: 95 },
      cells: [],
      fn: function (batch) {
        let link = `#gid=${sheetId}&range=B${batch.row.insert}`;
        let label = `Lab Materails`;
      ...
      data: [
        ...
        },
        {
          ...
          row: { min: 102, max: 107, insert: 105 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
        ...
        },
        {
          ...
          row: { min: 114, max: 118, insert: 116 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
        ...
        },
        {
          ...
          row: { min: 119, max: 125, insert: 122 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
...
Self.fillResearchOutputAvailabilityStatementDataSeerGenericPart = function (opts = {}, cb) {
  ...
  let lines = {
    ...
    softwares: softwares.map(function (item) {
      return {
        name: conf.settings.datasetsLinks[`DataSeer Generic`]
          ? Self.buildDatasetLink(documentId, documentToken, item.id, item.name)
          : item.name,
  ...
  lines.codes = lines.codes.length ? lines.codes : [{ name: ``, URL: ``, identifier: ``, notes: `` }];
  lines.softwares = lines.softwares.length ? lines.softwares : [{ name: ``, URL: ``, identifier: ``, notes: `` }];
  ...
  let actions = [
    ...
    function (next) {
      ...
      if (conf.settings.ROAS[`DataSeer Generic`]) {
        ...
        requests.push(Self.buildRequestInsertRows(sheetId, 11, 11 + lines.softwares.length - 1, true));
        requests.push(Self.buildRequestInsertRows(sheetId, 10, 10 + lines.codes.length - 1, true));
...
Self.insertDataInASAPSheets = function (opts = {}, cb) {
  ...
  let summary = _.get(data, `summary`, {});
  let dataTypesInfos = _.get(data, `dataTypesInfos`, {});
  let datasetsMetadata = _.get(data, `datasetsMetadata`, {});
  let filteredDatasets = [
    // Deposit to appropriate repository and cite PID in text
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP`].status.datasets[`4`].label;
    }),
    // Cite dataset unique identifier in text
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP`].status.datasets[`5`].label;
    }),
    // Verify dataset citation
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP`].status.datasets[`3`].label;
    }),
    // Done - Cited correctly
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP`].status.datasets[`0`].label;
    }),
    // Not required, but recommended to include the representative image/video files in raw format with dataset upload.
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP`].status.datasets[`1`].label;
    }),
    // Confirmatory and Quality Control datasets are not required to be included in your dataset upload.
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP`].status.datasets[`2`].label;
    })
  ];
  let filteredCodes = [
    // Upload code to Zenodo and provide DOI
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP`].status.codes[subType][`2`].label;
    }),
    // Verify citation
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP`].status.codes[subType][`1`].label;
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP`].status.codes[subType][`0`].label;
    })
  ];
  let filteredSoftwares = [
    // Include missing information in citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP`].status.softwares[subType][`2`].label;
    }),
    // Verify citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP`].status.softwares[subType][`1`].label;
    }),
    // Done - Citation meets minimum standards
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP`].status.softwares[subType][`0`].label;
    })
  ];
  let filteredMaterials = [
    // Assign RRID and cite in text
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP`].status.reagents[`3`].label;
    }),
    // Check for an existing RRID
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP`].status.reagents[`2`].label;
    }),
    // Verify citation
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP`].status.reagents[`1`].label;
    }),
    // Done - Cited correctly
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP`].status.reagents[`0`].label;
    })
  ];
  let filteredProtocols = [
    // Deposit in repository and cite DOI in text
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP`].status.protocols[subType][`2`].label;
    }),
    // Confirm citations meet minimum standards
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP`].status.protocols[subType][`3`].label;
    }),
    // Verify citation
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP`].status.protocols[subType][`1`].label;
    }),
    // Done - Cited correctly
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP`].status.protocols[subType][`0`].label;
    })
  ];
  let filteredDataObject = {
    datasets: {
    ...
    softwares: {
      filtered: filteredSoftwares,
      identified: filteredSoftwares.flat(),
      shared: filteredSoftwares.slice(2).flat()
    },
    materials: {
      filtered: filteredMaterials,
      identified: filteredMaterials.flat(),
        ...
        return Self.fillSummaryASAPSheet(
          {
            ...
            data: {
              ...
              protocols: {
                notes: datasetsMetadata.protocols.notes,
                new: {
              ...
              codeAndSoftware: {
                notes: datasetsMetadata.codeAndSoftware.notes,
                code: {
              ...
              materials: {
                notes: datasetsMetadata.materials.notes,
                new: {
              ...
              datasets: {
                notes: datasetsMetadata.datasets.notes,
                new: {
        ...
        return Self.fillDataObjectsTabsASAPSheet(
          {
            ...
            sheets: {
              ...
              'codes': { id: codeAndSoftwareReportSheetId, name: `Code and Software` },
              'softwares': { id: codeAndSoftwareReportSheetId, name: `Code and Software` },
...
Self.fillResearchOutputAvailabilityStatementAndOtherInformationASAPPart = function (opts = {}, cb) {
  ...
  let roas = {
    ...
    codes: _.get(data, `roas.codes`, []),
    softwares: _.get(data, `roas.softwares`, []),
    materials: _.get(data, `roas.materials`, []),
    datasets: _.get(data, `roas.datasets`, [])
  };
  let otherInformation = {
    protocols: _.get(data, `otherInformation.protocols`, []),
    codes: _.get(data, `otherInformation.codes`, []),
    softwares: _.get(data, `otherInformation.softwares`, []),
  ...
  let animalModels = otherInformation.materials
    ...
    .map(function (item) {
      return {
        name: conf.settings.datasetsLinks[`ASAP`]
          ? Self.buildDatasetLink(documentId, documentToken, item.id, item.name)
          : item.name,
        URL: item.source ? `${item.source} ` : `` + item.catalogNumber ? `${item.catalogNumber} ` : ``,
        identifier: item.RRID,
  ...
  let lines = {
    ...
    codesAndSoftwares: roas.codes
      ...
      .concat(
        roas.softwares.map(function (item) {
          return {
            name: conf.settings.datasetsLinks[`ASAP`]
              ? Self.buildDatasetLink(documentId, documentToken, item.id, item.name)
              : item.name,
  ...
  // Default values (to avoid offset bug)
  lines.datasets = lines.datasets.length ? lines.datasets : [{ name: ``, URL: ``, identifier: ``, notes: `` }];
  lines.codesAndSoftwares = lines.codesAndSoftwares.length
    ? lines.codesAndSoftwares
    : [{ name: ``, URL: ``, identifier: ``, notes: `` }];
  lines.materials = lines.materials.length ? lines.materials : [{ name: ``, URL: ``, identifier: ``, notes: `` }];
  lines.protocols = lines.protocols.length ? lines.protocols : [{ name: ``, URL: ``, identifier: ``, notes: `` }];
  // Concat all data
  lines.all = lines.datasets.concat(lines.codesAndSoftwares).concat(lines.materials).concat(lines.protocols);
  let actions = [
    // Insert rows
    function (next) {
      let otherInformation = [];
      ...
      let researchOutputAvailabilityStatement = [
        ...
        Self.buildRequestInsertRows(sheetId, 41, 41 + lines.codesAndSoftwares.length - 1, true),
...
Self.fillDataObjectsTabsASAPSheet = function (opts = {}, cb) {
  ...
  let codesReportSheet = _.get(sheets, `codes`);
  let softwaresReportSheet = _.get(sheets, `softwares`);
...
Self.insertDataInASAPPPMISheets = function (opts = {}, cb) {
  ...
  let filteredDatasets = [
    ...
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.datasets[`4`].label;
    }),
    // Cite PPMI dataset unique identifier in text (reuse)
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.datasets[`6`].label;
    }),
    // Cite dataset unique identifier in text
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.datasets[`5`].label;
    }),
    // Verify dataset citation
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.datasets[`3`].label;
    }),
    // Done - Cited correctly
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.datasets[`0`].label;
    }),
    // Not required, but recommended to include the representative image/video files in raw format with dataset upload.
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.datasets[`1`].label;
    }),
    // Confirmatory and Quality Control datasets are not required to be included in your dataset upload.
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.datasets[`2`].label;
    })
  ];
  let filteredCodes = [
    // Upload code to Zenodo and provide DOI
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-PPMI`].status.codes[subType][`2`].label;
    }),
    // Verify citation
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-PPMI`].status.codes[subType][`1`].label;
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-PPMI`].status.codes[subType][`0`].label;
    })
  ];
  let filteredSoftwares = [
    // Include missing information in citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-PPMI`].status.softwares[subType][`2`].label;
    }),
    // Verify citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-PPMI`].status.softwares[subType][`1`].label;
    }),
    // Done - Citation meets minimum standards
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-PPMI`].status.softwares[subType][`0`].label;
    })
  ];
  let filteredMaterials = [
    // Assign RRID and cite in text
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.reagents[`3`].label;
    }),
    // Check for an existing RRID
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.reagents[`2`].label;
    }),
    // Verify citation
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.reagents[`1`].label;
    }),
    // Done - Cited correctly
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP-PPMI`].status.reagents[`0`].label;
    })
  ];
  let filteredProtocols = [
    // Deposit in repository and cite DOI in text
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-PPMI`].status.protocols[subType][`2`].label;
    }),
    // Confirm citations meet minimum standards
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-PPMI`].status.protocols[subType][`3`].label;
    }),
    // Verify citation
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-PPMI`].status.protocols[subType][`1`].label;
    }),
    // Done - Cited correctly
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-PPMI`].status.protocols[subType][`0`].label;
    })
  ];
  let filteredDataObject = {
    datasets: {
...
Self.fillResearchOutputAvailabilityStatementAndOtherInformationASAPPPMIPart = function (opts = {}, cb) {
  ...
  let animalModels = otherInformation.materials
    ...
    .map(function (item) {
      return {
        name: conf.settings.datasetsLinks[`ASAP-PPMI`]
          ? Self.buildDatasetLink(documentId, documentToken, item.id, item.name)
          : item.name,
        URL: item.source ? `${item.source} ` : `` + item.catalogNumber ? `${item.catalogNumber} ` : ``,
        identifier: item.RRID,
...
Self.insertDataInASAPGP2Sheets = function (opts = {}, cb) {
  ...
  let filteredDatasets = [
    // Deposit to appropriate repository and cite PID in text
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-GP2`].status.datasets[`4`].label;
    }),
    // Cite dataset unique identifier in text
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-GP2`].status.datasets[`5`].label;
    }),
    // Verify dataset citation
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-GP2`].status.datasets[`3`].label;
    }),
    // Done - Cited correctly
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-GP2`].status.datasets[`0`].label;
    }),
    // Not required, but recommended to include the representative image/video files in raw format with dataset upload.
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-GP2`].status.datasets[`1`].label;
    }),
    // Confirmatory and Quality Control datasets are not required to be included in your dataset upload.
    datasets.filter(function (item) {
      return item.status === RULES[`ASAP-GP2`].status.datasets[`2`].label;
    })
  ];
  let filteredCodes = [
    // Upload code to Zenodo and provide DOI
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-GP2`].status.codes[subType][`2`].label;
    }),
    // Verify citation
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-GP2`].status.codes[subType][`1`].label;
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-GP2`].status.codes[subType][`0`].label;
    })
  ];
  let filteredSoftwares = [
    // Include missing information in citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-GP2`].status.softwares[subType][`2`].label;
    }),
    // Verify citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-GP2`].status.softwares[subType][`1`].label;
    }),
    // Done - Citation meets minimum standards
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-GP2`].status.softwares[subType][`0`].label;
    })
  ];
  let filteredMaterials = [
    // Assign RRID and cite in text
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP-GP2`].status.reagents[`3`].label;
    }),
    // Check for an existing RRID
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP-GP2`].status.reagents[`2`].label;
    }),
    // Verify citation
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP-GP2`].status.reagents[`1`].label;
    }),
    // Done - Cited correctly
    reagents.filter(function (item) {
      return item.status === RULES[`ASAP-GP2`].status.reagents[`0`].label;
    })
  ];
  let filteredProtocols = [
    // Deposit in repository and cite DOI in text
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-GP2`].status.protocols[subType][`2`].label;
    }),
    // Confirm citations meet minimum standards
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-GP2`].status.protocols[subType][`3`].label;
    }),
    // Verify citation
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-GP2`].status.protocols[subType][`1`].label;
    }),
    // Done - Cited correctly
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === RULES[`ASAP-GP2`].status.protocols[subType][`0`].label;
    })
  ];
  let filteredDataObject = {
    datasets: {
...
Self.fillResearchOutputAvailabilityStatementAndOtherInformationASAPGP2Part = function (opts = {}, cb) {
  ...
  let animalModels = otherInformation.materials
    ...
    .map(function (item) {
      return {
        name: conf.settings.datasetsLinks[`ASAP-GP2`]
          ? Self.buildDatasetLink(documentId, documentToken, item.id, item.name)
          : item.name,
        URL: item.source ? `${item.source} ` : `` + item.catalogNumber ? `${item.catalogNumber} ` : ``,
        identifier: item.RRID,
...
/**
 * This function will build the URL to a given dataset in the new GUI
 ...
 */
Self.buildDatasetLink = function (documentId, documentToken, datasetId, label) {
  ...
  if (typeof datasetId === `undefined`) throw new Error(`Missing required data: datasetId`);
  ...
  return `=LIEN_HYPERTEXTE("${Url.build(`/documents/${documentId}`, {
    token: documentToken,
    view: `datasets`,
    selectedDataObjectId: datasetId
  })}";"${label}")`;
};
