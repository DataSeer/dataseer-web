...
Self.fillResearchOutputAvailabilityStatementAndOtherInformationASAPPart = function (opts = {}, cb) {
  ...
  let actions = [
    ...
    function (next) {
      ...
      researchOutputAvailabilityStatement.push(
        Self.buildRequestUpdateBorders(sheetId, 41, 41 + lines.all.length - 1, 6, 8, GRAY_BORDERS)
      );
      researchOutputAvailabilityStatement.push(
        Self.buildRequestUpdateBorders(sheetId, 41, 41 + lines.all.length - 1, 5, 6, BLACK_BORDERS)
      );
  ...
  let actions = [
    ...
    function (next) {
      ...
      let data = [
        ...
        Self.buildRequestUpdateCell(`'Summary Sheet'!F37`, [DAS])
      ];
      return gSheets.spreadsheets.values.batchUpdate(
        {
          // The ID of the spreadsheet to update.
