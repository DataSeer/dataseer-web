        ...
        return Self.fillComplianceReportAmNatSheet(
          {
            ...
            data: {
              ...
              summary: summary,
              datasets: datasets,
              codeAndSoftware: codes.concat(softwares)
            }
          },
          function (err, res) {
            return next(err);
...
Self.fillComplianceReportAmNatSheet = function (opts = {}, cb) {
      ...
      data: [
        {
          ...
          data: codeAndSoftware.map(function (item) {
            return {
              reuse: item.reuse,
              name: item.name,
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    let datatype = dataset
      ? dataset.type.url
        ? `=LIEN_HYPERTEXTE("${dataset.type.url}";"Datasets - ${dataset.type.label}")`
        : `Datasets - ${dataset.type.label}`
      : `unknow`;
