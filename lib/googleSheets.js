...
Self.fillResearchOutputAvailabilityStatementAndOtherInformationASAPPart = function (opts = {}, cb) {
  ...
  let actions = [
    ...
    function (next) {
      ...
      let lastLinesStartIndex =
        39 +
        (animalModels.length > 0 ? animalModels.length - 1 : 0) +
        (cellLines.length > 0 ? cellLines.length - 1 : 0) +
        6;
      let lastLinesEndIndex = lastLinesStartIndex + 8 + lines.all.length;
      let deleteOffsetOfSections = [
        ...
        Self.buildRequestDeleteRows(sheetId, lastLinesStartIndex, lastLinesEndIndex) // Delete last lines
      ];
