...
const changesConf = require(`../conf/changes.json`);
...
/**
 * This function will build the changes from DS logs and return the file ID (google file ID)
 * @param {object} opts - Options available
 * @param {string} opts.filename - filename (of the report)
 * @param {string} opts.kind - What kind of report (available values : ORG or DS)
 * @param {array} opts.data - List of data
 * @param {function} cb - Callback function(err, res) (err: error process OR null, res: google file ID OR undefined)
 * @returns {undefined} undefined
 */
Self.buildChangFromDSLogs = function (opts, cb) {
  // Check all required data
  let kind = _.get(opts, `kind`);
  ...
  return Self.createSheetsFile(
    ...
    function (err, id) {
      ...
      if (kind === `ORG`)
        return Self.insertDataInChangesFromDSLogsFile({ spreadsheetId: id, data: opts.data[0] }, function (err) {
          if (err) return cb(err);
          return cb(null, id);
        });
      else if (kind === `DS`)
...
Self.insertDataInChangesFromDSLogsFile = function (opts = {}, cb) {
  ...
  return Self.getSheets({ spreadsheetId: spreadsheetId }, function (err, sheets) {
    ...
    let formattedDataSheetId = Self.getSheetId(sheets, `Formatted Data`);
    let filteredDataSheetId = Self.getSheetId(sheets, `Filtered Data`);
    let duplicatesSheetId = Self.getSheetId(sheets, `Dupicates`);
    let actions = [
      // fill 'Imported Data'
      function (next) {
      ...
      // fill 'Formatted Data' & 'Filtered Data' & 'Dupicates'
      function (next) {
        if (formattedDataSheetId instanceof Error) return next(formattedDataSheetId);
        ...
        return Self.insertLinesChangesFromDSLogsFileSheet(
          {
            ...
            data: {
              sheets: {
                'Formatted Data': formattedDataSheetId,
  ...
  return gSheets.spreadsheets.values.batchUpdate(
    {
      ...
      requestBody: {
        ...
        data: [Self.buildRequestPasteData(sheetId, data, rowIndex, 0, 2)],
  ...
  return gSheets.spreadsheets.values.batchUpdate(
    {
      ...
      requestBody: {
        // request body parameters
        data: [
          Self.buildRequestInsertRows(data.sheets[`Formatted Data`], 2, data.nbLines + 1, false),
          Self.buildRequestInsertRows(data.sheets[`Filtered Data`], 2, data.nbLines + 1, false),
          Self.buildRequestInsertRows(data.sheets[`Duplicates`], 2, data.nbLines + 1, false)
        ],
        valueInputOption: `USER_ENTERED`
      }
    },
  ...
  let actions = [
    ...
    function (next) {
      ...
      for (let i = 0; i < authors.length; i++) {
        ...
        data.push(Self.buildRequestUpdateCell(`'${sheetName}'!G${rowIndex}`, [orcid]));
        data.push(
  ...
  let actions = [
    ...
    function (next) {
      ...
      for (let i = 0; i < authors.length; i++) {
        ...
        data.push(Self.buildRequestUpdateCell(`'Detailed Report'!G${rowIndex}`, [orcid]));
        data.push(
  ...
  let actions = [
    ...
    function (next) {
      ...
      let deleteOffsetOfSections = [
        Self.buildRequestDeleteRange(sheetId, 39, 40 + lines.all.length, 0, 4, `ROWS`), // Other Information section
  ...
  let actions = [
    ...
    function (next) {
      ...
      for (let i = 0; i < authors.length; i++) {
        ...
        data.push(
          Self.buildRequestUpdateCell(`'Authors and Affiliation'!D${rowIndex}`, [authors[i].orcid.currentValue])
        );
        data.push(
...
Self.buildRequestPasteData = function (
  ...
  if (typeof sheet_id === `undefined`) throw new Error(`Missing required data: sheet_id`);
...
Self.getCustomSoftware = function (cb) {
  ...
  const spreadsheetRange = customSoftwareConf.spreadsheetRange;
  if (typeof fileId === `undefined`) throw new Error(`Missing required data: fileId`);
  if (typeof spreadsheetName === `undefined`) throw new Error(`Missing required data: spreadsheetName`);
  ...
  return gSheets.spreadsheets.values.get({ spreadsheetId: fileId, range: range }, function (err, res) {
    ...
    for (var i = 1; i < res.data.values.length; i++) {
      ...
      if (row[0] !== ``)
        software.push({
          'name': name,
          'suggestedEntity': row[1].replace(/\s+/gim, ` `),
          'suggestedURL': row[2].replace(/\s+/gim, ` `),
          'suggestedRRID': row[3].replace(/\s+/gim, ` `),
          'RRID': row[4].replace(/\s+/gim, ` `),
          'isCommandLine': isCommandLine,
          'parentLanguage': row[6].replace(/\s+/gim, ` `),
