          ...
          requestBody: {
            ...
            data: [
              ...
              Self.buildRequestUpdateCell(`'Compliance Report'!D6`, [
                `=LIEN_HYPERTEXTE("${metadata.dataSeerLink.url}&fromReport=true";"${metadata.dataSeerLink.label}")`
              ]),
...
Self.insertDataInDataSeerGenericSheets = function (opts = {}, cb) {
  ...
  let filteredDatasets = [
    //      a              b                    c                      d               e            f
    // item.reuse      item.qc      item.representativeImage      item.issues      item.URL      item.PID
    ...
    datasets.filter(function (item) {
      ...
      /*
        (!item.reuse && !item.qc && !item.representativeImage && !item.issues && !item.URL && !item.PID)
      */
      return !item.reuse && !item.qc && !item.representativeImage && !item.issues && !item.URL && !item.PID;
    }),
    ...
    datasets.filter(function (item) {
      ...
      /*
        (item.reuse && !item.qc && item.representativeImage && item.issues && item.URL && !item.PID) ||
        (!item.reuse && item.issues && !item.URL && !item.PID) ||
        (item.qc && item.issues && !item.URL && !item.PID) ||
        (!item.representativeImage && item.issues && !item.URL && !item.PID)
      */
      return (
        (!item.reuse && item.qc && !item.representativeImage && item.issues && !item.URL && !item.PID) ||
        (!item.reuse && item.qc && item.representativeImage && item.issues && !item.URL && !item.PID) ||
        (item.reuse && item.qc && !item.representativeImage && item.issues && !item.URL && !item.PID) ||
        (item.reuse && item.qc && item.representativeImage && item.issues && !item.URL && !item.PID) ||
        (item.reuse && !item.qc && item.representativeImage && item.issues && Params.isURL(item.URL) && !item.PID) ||
        (!item.reuse && !item.qc && item.representativeImage && item.issues && !item.URL && !item.PID) ||
        (!item.reuse && !item.qc && !item.representativeImage && item.issues && !item.URL && !item.PID) ||
        (item.reuse && !item.qc && !item.representativeImage && item.issues && !item.URL && !item.PID)
      );
    ...
    datasets.filter(function (item) {
      ...
      /*
        (item.reuse && !item.representativeImage && !item.issues && !item.URL && item.PID) ||
        (!item.reuse && item.qc && !item.issues && item.URL && !item.PID) ||
        (!item.qc && !item.representativeImage && !item.issues && !item.URL && item.PID) ||
        (item.representativeImage && !item.issues && item.URL && !item.PID)
      */
      return (
        (!item.reuse && !item.qc && !item.representativeImage && !item.issues && !item.URL && item.PID) ||
        ...
        (item.reuse && item.qc && item.representativeImage && !item.issues && item.URL && !item.PID) ||
        (item.reuse && !item.qc && item.representativeImage && !item.issues && item.URL && !item.PID) ||
        (!item.reuse && item.qc && item.representativeImage && !item.issues && item.URL && !item.PID) || // URL === "supplementary information"
        (!item.reuse && !item.qc && item.representativeImage && !item.issues && item.URL && !item.PID) || // URL === "supplementary information"
        (item.reuse && !item.qc && !item.representativeImage && !item.issues && !item.URL && item.PID) ||
        (item.reuse && !item.qc && !item.representativeImage && !item.issues && item.URL && !item.PID) ||
        (item.reuse && !item.qc && !item.representativeImage && !item.issues && item.URL && item.PID) ||
  ...
  let filteredCodes = [
    ...
    codes.filter(function (item) {
      ...
      */
      return !item.issues && !item.DOI && !item.URL;
    }),
    // Verify citation
    codes.filter(function (item) {
      /* (a * ! c) */
      ...
      */
      return (
        (item.issues && !item.DOI && !item.URL) ||
        (item.issues && (Params.isGithubRepository(item.DOI) || Params.isGithubRepository(item.URL)))
      );
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      ...
      /*
        ...
        (!item.issues && !item.comments && item.DOI)
      */
      return !item.issues && (Params.isURL(item.DOI) || Params.isURL(item.URL));
    })
    //         a             b              c
    // item.issues     item.comments     item.DOI
  ];
  let filteredSoftwares = [
    ...
    softwares.filter(function (item) {
      ...
      /*
        // Not sure so keep old filter
        (!item.issues && item.comments && !item.URL) ||
        (!item.issues && !item.comments && item.URL)
      */
      return (
        (!item.reuse &&
          !item.version &&
          !item.issues &&
          Params.isGithubRepository(item.URL) &&
          !item.RRID &&
          !item.suggestedEntity) ||
        (!item.reuse && item.version && !item.issues && !item.URL && !item.RRID && !item.suggestedEntity) ||
        (!item.reuse &&
          item.version &&
          !item.issues &&
          Params.isGithubRepository(item.URL) &&
    ...
    softwares.filter(function (item) {
      ...
      /*
        ...
        (item.reuse && !item.issues && !item.URL && !item.RRID) ||
      ...
      */
      return (
        ...
        (item.reuse &&
          ...
          Params.isGithubRepository(item.URL) &&
          !item.RRID &&
          item.suggestedEntity === `n/a`) ||
        (item.reuse && !item.version && !item.issues && !item.URL && !item.RRID && !item.suggestedEntity) ||
        (item.reuse && item.version && !item.issues && Params.isURL(item.URL) && !item.RRID && !item.suggestedEntity)
      );
    }),
    // Verify citation
    softwares.filter(function (item) {
      ...
      /*
        ...
        (item.issues && item.URL && !item.suggestedEntity) ||
    ...
    softwares.filter(function (item) {
      ...
      /*
        (item.reuse && item.version && !item.issues && item.URL && !item.RRID && item.suggestedEntity) ||
        (item.version && !item.issues && item.URL && item.RRID && !item.suggestedEntity)
      */
      return (
        (item.reuse && item.version && !item.issues && item.URL && item.RRID && !item.suggestedEntity) ||
        (!item.reuse && item.version && !item.issues && item.URL && item.RRID && !item.suggestedEntity)
    ...
    softwares.filter(function (item) {
      ...
      /*
        (item.reuse && item.version && !item.issues && item.URL && !item.RRID) ||
        (!item.version && !item.issues && item.RRID && !item.suggestedEntity) ||
        (!item.issues && !item.URL && item.RRID && !item.suggestedEntity)
      */
      return (
        (item.reuse && item.version && !item.issues && item.URL && !item.RRID && item.suggestedEntity === `n/a`) ||
        (item.reuse && !item.version && !item.issues && !item.URL && item.RRID && !item.suggestedEntity) ||
        (item.reuse && !item.version && !item.issues && item.URL && item.RRID && !item.suggestedEntity) ||
        (item.reuse && item.version && !item.issues && item.URL && !item.RRID && !item.suggestedEntity) ||
        (item.reuse && item.version && !item.issues && !item.URL && item.RRID && !item.suggestedEntity) ||
  ...
  let filteredProtocols = [
    ...
    protocols.filter(function (item) {
      ...
      */
      return (
        item.reuse && !item.issues && (!item.DOI || !Params.isURL(item.DOI)) && (!item.URL || !Params.isURL(item.URL))
      );
    }),
    // Verify citation
    protocols.filter(function (item) {
    ...
    protocols.filter(function (item) {
      ...
      */
      return (
        !item.issues &&
        (item.DOI.indexOf(`Manufacturer's Instructions`) > -1 ||
          Params.isURL(item.DOI) ||
          item.URL.indexOf(`Manufacturer's Instructions`) > -1 ||
          Params.isURL(item.URL))
      );
    })
    //         a             b             c
        ...
        return Self.fillResearchOutputAvailabilityStatementGenericPart(
          {
            ...
            data: {
              ...
              softwares: filteredDataObject.softwares.filtered[3]
                .concat(filteredDataObject.softwares.filtered[4])
                .filter(function (item) {
                  return !item.reuse;
                }),
  ...
  let batches = [
          ...
          data: protocols[0].map(function (item) {
            return {
              ...
              sentences: item.sentences
                ...
                .join(` `),
              repository: (item.DOI ? `${item.DOI} ` : ``) + item.URL
            };
          })
        },
        {
      ...
      data: [
        {
          ...
          // Upload code to a repository and provide DOI
    ...
    // datasets
    {
      ...
      fn: function (batch) {
        let link = `#gid=${sheetId}&range=B${batch.row.insert}`;
        let label = `Datasets`;
        return Self.buildRequestUpdateCell(`'Detailed Report'!B8`, [`=LIEN_HYPERTEXTE("${link}", "${label}")`]);
      },
      data: [
        {
          merges: [
...
Self.fillResearchOutputAvailabilityStatementGenericPart = function (opts = {}, cb) {
  ...
  let lines = {
    orcids: orcids.map(function (item) {
      return { name: item, URL: ``, notes: `` };
    }),
    datasets: datasets.map(function (item) {
      return { name: item.name, URL: item.URL ? `${item.URL} ` : `` + item.PID, notes: item.comments };
    }),
    codes: codes.map(function (item) {
      return { name: item.name, URL: (item.DOI ? `${item.DOI} ` : ``) + item.URL, notes: item.comments };
    }),
    softwares: softwares.map(function (item) {
      return { name: item.name, URL: item.URL ? `${item.URL} ` : `` + item.RRID, notes: item.comments };
    }),
    materials: materials.map(function (item) {
      return {
        name: item.name,
        URL: item.source ? `${item.source} ` : `` + item.catalogNumber ? `${item.catalogNumber} ` : `` + item.RRID,
        notes: item.comments
      };
    }),
  ...
  lines.orcids = lines.orcids.length ? lines.orcids : [{ name: ``, URL: ``, notes: `` }];
  lines.datasets = lines.datasets.length ? lines.datasets : [{ name: ``, URL: ``, notes: `` }];
  lines.codes = lines.codes.length ? lines.codes : [{ name: ``, URL: ``, notes: `` }];
  lines.softwares = lines.softwares.length ? lines.softwares : [{ name: ``, URL: ``, notes: `` }];
  lines.materials = lines.materials.length ? lines.materials : [{ name: ``, URL: ``, notes: `` }];
  lines.protocols = lines.protocols.length ? lines.protocols : [{ name: ``, URL: ``, notes: `` }];
  // Concat all data
  ...
  let actions = [
    ...
    function (next) {
      let requests = [
        Self.buildRequestInsertRows(sheetId, 13, 13 + lines.orcids.length - 1, true),
        Self.buildRequestInsertRows(sheetId, 12, 12 + lines.protocols.length - 1, true),
        Self.buildRequestInsertRows(sheetId, 11, 11 + lines.materials.length - 1, true),
        Self.buildRequestInsertRows(sheetId, 10, 10 + lines.softwares.length - 1, true),
        Self.buildRequestInsertRows(sheetId, 9, 9 + lines.codes.length - 1, true),
        Self.buildRequestInsertRows(sheetId, 8, 8 + lines.datasets.length - 1, true),
        Self.buildRequestMergeCells(sheetId, 8, 8 + lines.all.length - 1, 1, 5),
        Self.buildRequestMergeCells(sheetId, 8, 8 + lines.all.length - 1, 6, 9),
        Self.buildRequestMergeCells(sheetId, 8, 8 + lines.all.length - 1, 9, 12),
        Self.buildRequestUpdateBorders(sheetId, 8, 8 + lines.all.length - 1, 1, 12)
      ];
      return gSheets.spreadsheets.batchUpdate(
        {
          // The ID of the spreadsheet to update.
    ...
    function (next) {
      ...
      let data = [
        ...
        Self.buildRequestUpdateCell(`'Detailed Report'!F4`, [
          `=LIEN_HYPERTEXTE("${dataSeerLink.url}&fromReport=true";"${dataSeerLink.label}")`
        ]),
...
Self.insertDataInASAPSheets = function (opts = {}, cb) {
  ...
  let filteredDatasets = [
    // Deposit to appropriate repository and cite PID in text
    datasets.filter(function (item) {
      return !item.reuse && !item.URL && !item.qc && !item.representativeImage && !item.issues;
    }),
    // Cite dataset unique identifier in text
    datasets.filter(function (item) {
      return item.reuse && !item.URL && !item.qc && !item.representativeImage && !item.issues;
    ...
    datasets.filter(function (item) {
      return (
        (item.URL && !item.qc && !item.issues) ||
        (item.URL && item.representativeImage && !item.issues) ||
        (item.URL && item.reuse && !item.issues)
      );
    }),
    // Not required, but recommended to include the representative image/video files in raw format with dataset upload.
    datasets.filter(function (item) {
  ...
  let filteredCodes = [
    // Upload code to Zenodo and provide DOI
    codes.filter(function (item) {
      return !item.reuse && !item.issues && (!item.DOI || Params.isGithubRepository(item.URL));
    }),
    // Verify citation
    codes.filter(function (item) {
  ...
  let filteredSoftwares = [
    ...
    softwares.filter(function (item) {
      return (
        (item.reuse && !item.RRID && !item.URL && !item.issues) ||
    ...
    // Done - Cited correctly
    softwares.filter(function (item) {
      return item.RRID && item.version && !item.issues && item.URL;
    }),
    // None - Citation meets minimum standards
    softwares.filter(function (item) {
      return (
        (item.RRID && !item.version && !item.issues) ||
        (item.RRID && !item.issues && !item.URL) ||
        (item.reuse && !item.RRID && item.version && !item.issues && (item.URL || item.suggestedEntity))
      );
    })
  ];
  let filteredMaterials = [
  ...
  let filteredProtocols = [
    // Deposit in repository and cite DOI in text
    protocols.filter(function (item) {
      return !item.reuse && !item.issues && !item.DOI && !item.URL;
    }),
    // Include missing information in citation
    protocols.filter(function (item) {
      return item.reuse && !item.issues && (!item.DOI || !Params.isURL(item.URL));
    }),
    // Verify citation
    protocols.filter(function (item) {
      return item.issues;
    ...
    protocols.filter(function (item) {
      return (
        !item.issues &&
        (item.DOI === `Manufacturer's Instructions` ||
          Params.isURL(item.DOI) ||
          item.URL === `Manufacturer's Instructions` ||
          Params.isURL(item.URL))
      );
    })
  ];
      ...
      data: [
        {
          ...
          row: { min: 123, max: 128, insert: 126 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
            { column: `H`, property: `URL` },
            { column: `I`, property: `DOI` },
        ...
        },
        {
          ...
          row: { min: 129, max: 134, insert: 132 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
        ...
        },
        {
          ...
          row: { min: 135, max: 140, insert: 138 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
        ...
        },
        {
          ...
          row: { min: 141, max: 145, insert: 143 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
    ...
    // lab materials
    {
      ...
      data: [
        ...
        },
        {
          ...
          row: { min: 104, max: 109, insert: 107 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
        ...
        },
        {
          ...
          row: { min: 110, max: 114, insert: 112 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
      ...
      data: [
        {
          ...
          cells: [
            ...
            { column: `G`, property: `sentences` },
            { column: `J`, property: `URL` },
            { column: `K`, property: `PID` },
            { column: `L`, property: `notes` },
            { column: `O`, property: `datatype` }
          ],
          data: datasets[0].map(function (item) {
            return {
              reuse: item.reuse,
