 ...
 * @param {object} opts.data.dataTypesInfo - dataTypesInfo
...
Self.insertDataInUniversalJournalReportSheets = function (opts = {}, cb) {
  ...
  let dataTypesInfo = _.get(data, `dataTypesInfo`, {});
...
Self.fillComplianceReportUniversalJournalReportSheet = function (opts = {}, cb) {
  ...
  let summary = _.get(data, `summary`, {});
  let dataTypesInfo = _.get(data, `dataTypesInfo`, {});
  let dataTypes = Object.keys(dataTypesInfo.dataTypes);
  let codes = _.get(data, `codes`, []);
  let softwares = _.get(data, `softwares`, []);
  let datasets = _.get(data, `datasets`, []);
          ...
          data: softwares.map(function (item) {
            return {
              ...
              name: conf.settings.dataObjectsLinks[`Universal Journal Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item.id, item.name)
                : item.name,
