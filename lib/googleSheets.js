...
Self.insertDataInDataSeerGenericSheets = function (opts = {}, cb) {
    ...
    datasets.filter(function (item) {
      ...
      */
      return (
        ...
        (item.reuse && item.qc && !item.representativeImage && item.issues && !item.DOI && !item.PID) ||
        (item.reuse && item.qc && item.representativeImage && item.issues && !item.DOI && !item.PID) ||
        (item.reuse && !item.qc && item.representativeImage && item.issues && Params.isURL(item.DOI) && !item.PID) ||
        (!item.reuse && !item.qc && item.representativeImage && item.issues && !item.DOI && !item.PID) ||
        (!item.reuse && !item.qc && !item.representativeImage && item.issues && !item.DOI && !item.PID) ||
        (item.reuse && !item.qc && !item.representativeImage && item.issues && !item.DOI && !item.PID)
      );
    ...
    datasets.filter(function (item) {
      ...
      */
      return (
        ...
        (item.reuse && item.qc && item.representativeImage && !item.issues && item.DOI && !item.PID) ||
        (item.reuse && !item.qc && item.representativeImage && !item.issues && item.DOI && !item.PID) ||
        (!item.reuse && item.qc && item.representativeImage && !item.issues && item.DOI && !item.PID) || // DOI === "supplementary information"
        (!item.reuse && !item.qc && item.representativeImage && !item.issues && item.DOI && !item.PID) || // DOI === "supplementary information"
        (item.reuse && !item.qc && !item.representativeImage && !item.issues && !item.DOI && item.PID) ||
        (item.reuse && !item.qc && !item.representativeImage && !item.issues && item.DOI && !item.PID) ||
        (item.reuse && !item.qc && !item.representativeImage && !item.issues && item.DOI && item.PID) ||
  ...
  let filteredCodes = [
    ...
    codes.filter(function (item) {
      ...
      */
      return (item.issues && !item.DOI) || (item.issues && Params.isGithubRepository(item.DOI));
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      /* (! a * b * ! c) + (! a * ! b * c) */
      /*
        ...
        (!item.issues && !item.comments && item.DOI)
      */
      return !item.issues && Params.isURL(item.DOI);
    })
    //         a             b              c
    // item.issues     item.comments     item.DOI
  ];
  let filteredSoftwares = [
      ...
      */
      return (
        (!item.reuse &&
          !item.version &&
          !item.issues &&
          Params.isGithubRepository(item.DOI) &&
          !item.RRID &&
    ...
    softwares.filter(function (item) {
      ...
      */
      return (
        ...
        (item.reuse && !item.version && !item.issues && !item.DOI && !item.RRID && !item.suggestedEntity) ||
        (item.reuse && item.version && !item.issues && Params.isURL(item.DOI) && !item.RRID && !item.suggestedEntity)
      );
    }),
    // Verify citation
    softwares.filter(function (item) {
    ...
    softwares.filter(function (item) {
      ...
      */
      return (
        (item.reuse && item.version && !item.issues && item.DOI && !item.RRID && item.suggestedEntity === `n/a`) ||
        (item.reuse && !item.version && !item.issues && !item.DOI && item.RRID && !item.suggestedEntity) ||
        (item.reuse && !item.version && !item.issues && item.DOI && item.RRID && !item.suggestedEntity) ||
        (item.reuse && item.version && !item.issues && item.DOI && !item.RRID && !item.suggestedEntity) ||
        (item.reuse && item.version && !item.issues && !item.DOI && item.RRID && !item.suggestedEntity) ||
          ...
          data: codes[0].map(function (item) {
            return {
              ...
              sentences: item.sentences
                ...
                .join(` `),
              DOI: !Params.isGithubRepository(item.DOI) ? item.DOI : ``,
              URL: Params.isGithubRepository(item.DOI) ? item.DOI : ``
            };
          })
        },
        {
...
Self.fillResearchOutputAvailabilityStatementGenericPart = function (opts = {}, cb) {
  ...
  let lines = {
    ...
    datasets: datasets.map(function (item) {
      return { name: item.name, URL: item.DOI ? `${item.DOI} ` : `` + item.PID, notes: item.comments };
    }),
    codes: codes.map(function (item) {
      return { name: item.name, URL: item.DOI, notes: item.comments };
    }),
    softwares: softwares.map(function (item) {
      return { name: item.name, URL: item.DOI ? `${item.DOI} ` : `` + item.RRID, notes: item.comments };
    }),
...
Self.insertDataInASAPSheets = function (opts = {}, cb) {
  ...
  let filteredCodes = [
    // Upload code to Zenodo and provide DOI
    codes.filter(function (item) {
      return !item.reuse && !item.issues && (!item.DOI || Params.isGithubRepository(item.DOI));
    }),
    // Verify citation
    codes.filter(function (item) {
