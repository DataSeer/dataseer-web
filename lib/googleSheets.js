    ...
    function (next) {
      ...
      for (let i = keys.length - 1; i >= 0; i--) {
        ...
        for (let j = 0; j < infos.list.length; j++) {
          ...
          data.push(Self.buildRequestUpdateCell(`'Compliance Report'!F${rowIndex}`, [dataset.comments]));
        }
      ...
      for (let i = 0; i < batches.codeAndSoftware.length; i++) {
        ...
        data.push(Self.buildRequestUpdateCell(`'Compliance Report'!E${rowIndex}`, [item.repository]));
        ...
        return Self.fillSummaryASAPSheet(
          {
            ...
            data: {
              ...
              protocols: {
                new: {
                  identified: sortedDatasets.protocols.new.length,
                  shared: sortedDatasets.protocols.new.filter(function (item) {
                    return !!item.DOI;
                  }).length
                },
                reuse: {
                  identified: sortedDatasets.protocols.reuse.length,
                  shared: sortedDatasets.protocols.reuse.filter(function (item) {
                    return !!item.DOI;
                  }).length
                }
              },
              codeAndSoftware: {
                code: {
                  identified: sortedDatasets.codes.length,
                  shared: sortedDatasets.codes.filter(function (item) {
                    return !!item.DOI;
                  }).length
                },
                software: {
                  identified: sortedDatasets.softwares.length,
                  shared: sortedDatasets.softwares.filter(function (item) {
                    return !!item.DOI;
                  }).length
                }
              },
              materialsResources: {
                new: {
                  identified: sortedDatasets.materials.new.length,
                  shared: sortedDatasets.materials.new.filter(function (item) {
                    return !!item.DOI;
                  }).length
                },
                reuse: {
                  identified: sortedDatasets.materials.reuse.length,
                  shared: sortedDatasets.materials.reuse.filter(function (item) {
                    return !!item.DOI;
                  }).length
                }
              },
              datasets: {
                new: {
                  identified: sortedDatasets.datasets.new.length,
                  shared: sortedDatasets.datasets.new.filter(function (item) {
                    return !!item.DOI;
                  }).length
                },
                reuse: {
                  identified: sortedDatasets.datasets.reuse.length,
                  shared: sortedDatasets.datasets.reuse.filter(function (item) {
                    return !!item.DOI;
                  }).length
                }
              }
...
Self.fillDetailedReportASAPSheet = function (opts = {}, cb) {
  ...
  let buildSummarySheetCellFormula = function (begin, end) {
    return `=SI(NB.SI('Detailed Report'!A${begin}:A${end},"=Yes"),"Yes",SI(NB.SI('Detailed Report'!A${begin}:A${end},"=Optional"),"=Optional","No"))`;
  };
  // Compute batches commands
  let batches = [
    // protocols
    {
      summarySheetCell: `'Action Required'!C6`,
      row: { min: 148, max: 148 },
      cells: [],
      data: [
        {
          ...
          row: { min: 149, max: 156, insert: 154 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `sentences` },
            { column: `H`, property: `repository` }
          ],
          data: protocols
            .filter(function (item) {
              return !item.reuse && !item.issue && !item.DOI;
            })
            .map(function (item) {
              return {
                reuse: item.reuse,
        ...
        },
        {
          ...
          row: { min: 173, max: 178, insert: 176 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `sentences` },
    ...
    // lab materials
    {
      summarySheetCell: `'Action Required'!C5`,
      row: { min: 108, max: 108 },
      cells: [],
      data: [
        {
          ...
          row: { min: 109, max: 116, insert: 114 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `issue` },
            { column: `F`, property: `name` },
            { column: `G`, property: `sentences` },
            { column: `H`, property: `catalog` },
            { column: `I`, property: `RRID` },
            { column: `J`, property: `dataType` },
            { column: `L`, property: `entity` },
            { column: `M`, property: `citation` }
          ],
          data: labMaterials
            .filter(function (item) {
              return !item.reuse && !item.RRID && !item.issue;
            })
            .map(function (item) {
              return {
                reuse: item.reuse,
        ...
        },
        {
          ...
          data: labMaterials
            .filter(function (item) {
              return (item.RRID && !item.issue) || (item.RRID && item.catalog);
            })
            .map(function (item) {
              return {
                reuse: item.reuse,
        ...
        },
        {
          ...
          data: labMaterials
            .filter(function (item) {
              return !item.RRID && item.catalog && !item.issue;
            })
            .map(function (item) {
              return {
                reuse: item.reuse,
    ...
    // software
    {
      summarySheetCell: `'Action Required'!C4`,
      row: { min: 68, max: 68 },
      cells: [],
      data: [
        {
          ...
          data: softwares
            .filter(function (item) {
              return (
                (!item.reuse && !item.version && !item.URL) ||
                (!item.RRID && !item.version && !item.reuse) ||
                (!item.reuse && !item.RRID && item.version && item.URL)
              );
            })
            .map(function (item) {
              return {
        ...
        },
        {
          ...
          data: softwares
            .filter(function (item) {
              return (
                (item.reuse && !item.RRID && !item.URL && !item.issue) ||
                (item.reuse && !item.RRID && !item.version && !item.issue)
              );
            })
            .map(function (item) {
              return {
        ...
        },
        {
          ...
          data: softwares
            .filter(function (item) {
              return (
                (item.RRID && !item.version && !item.issue) ||
                (item.RRID && !item.issue && !item.URL) ||
                (item.reuse && !item.RRID && item.version && !item.issue && item.URL)
              );
            })
            .map(function (item) {
              return {
        ...
        },
        {
          ...
          row: { min: 101, max: 107, insert: 104 },
          cells: [
            { column: `B`, property: `reuse` },
            { column: `C`, property: `version` },
            { column: `E`, property: `issue` },
          ...
          data: softwares
            .filter(function (item) {
              return item.RRID && item.version && !item.issue && item.URL;
            })
            .map(function (item) {
              return {
                reuse: item.reuse,
    ...
    // code
    {
      summarySheetCell: `'Action Required'!C3`,
      row: { min: 44, max: 44 },
      cells: [],
      data: [
        {
          ...
          data: codes
            .filter(function (item) {
              return !item.reuse && !item.issue && (!item.DOI || item.DOI.indexOf(`https://github.com/`) > -1);
            })
            .map(function (item) {
              return {
                reuse: item.reuse,
        ...
        },
        {
          ...
          data: codes
            .filter(function (item) {
              return !item.reuse && !item.issue && !item.DOI.indexOf(`https://github.com/`);
            })
            .map(function (item) {
              return {
                reuse: item.reuse,
  ...
  for (let i = 0; i < keys.length; i++) {
    let key = keys[i];
    let dataset = sortedDatasets[key].list[0];
    let datatype = dataset
      ? dataset.type.url
        ? `=LIEN_HYPERTEXTE("${dataset.type.url}";"${dataset.type.label}")`
        : dataset.type.label
      : `unknow`;
    ...
    function (batch, _next) {
      let actions = [
        ...
        function (n) {
          ...
          // Add formula in Summary Sheet if necessary
          if (batch.summarySheetCell)
            data.push(Self.buildRequestUpdateCell(batch.summarySheetCell, [buildSummarySheetCellFormula(begin, end)]));
          return gSheets.spreadsheets.values.batchUpdate(
            {
              // The ID of the spreadsheet to update.
              spreadsheetId: spreadsheetId,
  ...
  let actions = [
    ...
    function (next) {
      ...
      for (let i = 0; i < authors.length; i++) {
        ...
        data.push(Self.buildRequestUpdateCell(`'Authors and Affiliation'!E${rowIndex}`, [orcidsFromAPI]));
      }
      return gSheets.spreadsheets.values.batchUpdate(
        {
          // The ID of the spreadsheet to update.
...
Self.fillSummaryASAPSheet = function (opts = {}, cb) {
  ...
  let materialsResources = _.get(data, `materialsResources`, {
  ...
  let orcids = authors.filter(function (item) {
    return _.get(item, `orcid.fromTEI.length`, 0) > 0;
  }).length;
  let authorsNames = authors
    ...
    .join(`, `);
  let ASAPTeam = authors[authors.length - 1].name;
  let actions = [
    // Update cells & Add data validations
    function (next) {
      let date = new Date();
      let data = [
        ...
        Self.buildRequestUpdateCell(`'Summary Sheet'!B6`, [ASAPTeam]),
        ...
        Self.buildRequestUpdateCell(`'Summary Sheet'!B21`, [datasets.new.identified]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!C21`, [datasets.new.shared]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!B22`, [datasets.reuse.identified]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!C22`, [datasets.reuse.shared]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!B25`, [codeAndSoftware.code.identified]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!C25`, [codeAndSoftware.code.shared]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!B26`, [codeAndSoftware.software.identified]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!C26`, [codeAndSoftware.software.shared]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!B29`, [materialsResources.new.identified]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!C29`, [materialsResources.new.shared]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!B30`, [materialsResources.reuse.identified]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!C30`, [materialsResources.reuse.shared]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!B33`, [protocols.new.identified + protocols.reuse.identified]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!C33`, [protocols.new.shared + protocols.reuse.shared]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!B36`, [orcids]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!C36`, [orcids])
      ];
      return gSheets.spreadsheets.values.batchUpdate(
        {
          // The ID of the spreadsheet to update.
  ...
  let actions = [
    ...
    function (next) {
      ...
      for (let i = 0; i < batches.protocols.length; i++) {
        ...
        data.push(Self.buildRequestUpdateCell(`'Compliance Report'!E${rowIndex}`, [protocol.name]));
        data.push(
