...
Self.insertDataInSheets = function (opts = {}, cb) {
  ...
  let ASAPPieImage = _.get(data, `urls.ASAPPieImage`, ``);
  return Self.getSheets({ spreadsheetId: spreadsheetId }, function (err, sheets) {
    ...
    let actions = [
      ...
      function (next) {
        ...
        return Self.fillComplianceReportSheet(
          {
            spreadsheetId: spreadsheetId,
      ...
      function (next) {
        ...
        return Self.fillChartsSheet(
          {
            spreadsheetId: spreadsheetId,
            sheetId: chartsId,
            data: {
              ASAPPieImage: ASAPPieImage
            }
          },
          function (err, res) {
            return next(err);
 ...
 * @returns {undefined} undefined
 */
Self.fillChartsSheet = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `spreadsheetId`) === `undefined`)
    return cb(Error(`Missing required data: opts.spreadsheetId`));
  ...
  let data = _.get(opts, `data`, {});
  let ASAPPieImage = _.get(data, `ASAPPieImage`, ``);
  let _data = [Self.buildRequestUpdateCell(`'Charts'!A1`, [`=IMAGE("${ASAPPieImage}";4;600;1067)`])];
 ...
 * @returns {undefined} undefined
 */
Self.fillComplianceReportSheet = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `spreadsheetId`) === `undefined`)
    return cb(Error(`Missing required data: opts.spreadsheetId`));
  if (typeof _.get(opts, `sheetId`) === `undefined`) return cb(Error(`Missing required data: opts.sheetId`));
