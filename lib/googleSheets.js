...
Self.insertDataInDataSeerGenericSheets = function (opts = {}, cb) {
  ...
  let filteredCodes = [
    ...
    codes.filter(function (item) {
      ...
      return Self.getRuleKey(item.rules[`default`].rule) === RULES[`default`].status.codes[subType][`2`].key;
    }),
    // Verify citation
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`default`].rule) === RULES[`default`].status.codes[subType][`1`].key;
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`default`].rule) === RULES[`default`].status.codes[subType][`0`].key;
    })
  ];
  let filteredSoftwares = [
    ...
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`default`].rule) === RULES[`default`].status.softwares[subType][`2`].key;
    }),
    // Verify citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`default`].rule) === RULES[`default`].status.softwares[subType][`1`].key;
    }),
    // None - Citation meets minimum standards
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`default`].rule) === RULES[`default`].status.softwares[subType][`0`].key;
    })
  ];
...
Self.insertDataInASAPSheets = function (opts = {}, cb) {
  ...
  let filteredCodes = [
    ...
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP`].rule) === RULES[`ASAP`].status.codes[subType][`2`].key;
    }),
    // Verify citation
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP`].rule) === RULES[`ASAP`].status.codes[subType][`1`].key;
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP`].rule) === RULES[`ASAP`].status.codes[subType][`0`].key;
    })
  ];
  let filteredSoftwares = [
    // Include missing information in citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP`].rule) === RULES[`ASAP`].status.softwares[subType][`2`].key;
    }),
    // Verify citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP`].rule) === RULES[`ASAP`].status.softwares[subType][`1`].key;
    }),
    // Done - Citation meets minimum standards
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP`].rule) === RULES[`ASAP`].status.softwares[subType][`0`].key;
    })
  ];
  let filteredMaterials = [
    // Assign RRID and cite in text
...
Self.insertDataInASAPPPMISheets = function (opts = {}, cb) {
  ...
  let filteredCodes = [
    ...
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-PPMI`].rule) === RULES[`ASAP-PPMI`].status.codes[subType][`2`].key;
    }),
    // Verify citation
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-PPMI`].rule) === RULES[`ASAP-PPMI`].status.codes[subType][`1`].key;
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-PPMI`].rule) === RULES[`ASAP-PPMI`].status.codes[subType][`0`].key;
    })
  ];
  let filteredSoftwares = [
    // Include missing information in citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-PPMI`].rule) === RULES[`ASAP-PPMI`].status.softwares[subType][`2`].key;
    }),
    // Verify citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-PPMI`].rule) === RULES[`ASAP-PPMI`].status.softwares[subType][`1`].key;
    }),
    // Done - Citation meets minimum standards
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-PPMI`].rule) === RULES[`ASAP-PPMI`].status.softwares[subType][`0`].key;
    })
  ];
  let filteredMaterials = [
    // Assign RRID and cite in text
...
Self.insertDataInASAPGP2Sheets = function (opts = {}, cb) {
  ...
  let filteredCodes = [
    ...
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-GP2`].rule) === RULES[`ASAP-GP2`].status.codes[subType][`2`].key;
    }),
    // Verify citation
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-GP2`].rule) === RULES[`ASAP-GP2`].status.codes[subType][`1`].key;
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-GP2`].rule) === RULES[`ASAP-GP2`].status.codes[subType][`0`].key;
    })
  ];
  let filteredSoftwares = [
    // Include missing information in citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-GP2`].rule) === RULES[`ASAP-GP2`].status.softwares[subType][`2`].key;
    }),
    // Verify citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-GP2`].rule) === RULES[`ASAP-GP2`].status.softwares[subType][`1`].key;
    }),
    // Done - Citation meets minimum standards
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`ASAP-GP2`].rule) === RULES[`ASAP-GP2`].status.softwares[subType][`0`].key;
    })
  ];
  let filteredMaterials = [
    // Assign RRID and cite in text
