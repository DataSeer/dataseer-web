 ...
 * @param {string} opts.data.document - document data
 ...
 * @param {array} opts.data.code - code
...
Self.buildReport = function (opts = {}, cb) {
  ...
  for (let i = 0; i < opts.data.document.organizations.length; i++) {
    let organizationId = opts.data.document.organizations[i];
    if (typeof folders[organizationId] !== `undefined`) {
      folder = folders[organizationId];
      break;
    }
...
Self.insertDataInHHMISheets = function (opts = {}, cb) {
  ...
  let filteredDatasets = [
    ...
    datasets.filter(function (item) {
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.datasets[`4`].key;
    }),
    // Cite dataset unique identifier in text
    datasets.filter(function (item) {
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.datasets[`5`].key;
    }),
    // Verify dataset citation
    datasets.filter(function (item) {
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.datasets[`3`].key;
    }),
    ...
    datasets.filter(function (item) {
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.datasets[`1`].key;
    }),
  ...
  let filteredCode = [
    ...
    code.filter(function (item) {
      ...
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.code[subType][`2`].key;
    }),
    // Verify citation
    code.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.code[subType][`1`].key;
    }),
    // Done - Cited correctly
    code.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
  ...
  let filteredSoftware = [
    ...
    software.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.software[subType][`2`].key;
    }),
    // Verify citation
    software.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.software[subType][`1`].key;
    }),
  ...
  let filteredMaterials = [
    ...
    reagents.filter(function (item) {
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.reagents[`3`].key;
    }),
    // Check for an existing RRID
    reagents.filter(function (item) {
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.reagents[`2`].key;
    }),
    // Verify citation
    reagents.filter(function (item) {
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.reagents[`1`].key;
    }),
    // Done - Cited correctly
    reagents.filter(function (item) {
  ...
  let filteredProtocols = [
    ...
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.protocols[subType][`2`].key;
    }),
    // Confirm citations meet minimum standards
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.protocols[subType][`3`].key;
    }),
    // Verify citation
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return Self.getRuleKey(item.rules[`HHMI`].rule) === RULES[`HHMI`].status.protocols[subType][`1`].key;
    }),
    // Done - Cited correctly
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
  ...
  return Self.getSheets({ spreadsheetId: spreadsheetId }, function (err, sheets) {
    ...
    let actions = [
      ...
      function (next) {
        if (summarySheetId instanceof Error) return next(summarySheetId);
        return Self.fillResearchOutputAvailabilityStatementAndOtherInformationHHMIPart(
          {
            spreadsheetId: spreadsheetId,
            sheetId: summarySheetId,
            data: {
      ...
      function (next) {
        ...
        return Self.fillAuthorsAndAffiliationHHMISheet(
          {
            spreadsheetId: spreadsheetId,
      ...
      function (next) {
        ...
        return Self.fillDataObjectsTabsHHMISheet(
          {
            spreadsheetId: spreadsheetId,
            sheets: {
      ...
      function (next) {
        ...
        return Self.fillChartsHHMISheet(
          {
            spreadsheetId: spreadsheetId,
 ...
 * @returns {undefined} undefined
 */
Self.fillResearchOutputAvailabilityStatementAndOtherInformationHHMIPart = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `spreadsheetId`) === `undefined`)
    return cb(Error(`Missing required data: opts.spreadsheetId`));
 ...
 * @returns {undefined} undefined
 */
Self.fillChartsHHMISheet = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `spreadsheetId`) === `undefined`)
    return cb(Error(`Missing required data: opts.spreadsheetId`));
  if (typeof _.get(opts, `sheetId`) === `undefined`) return cb(Error(`Missing required data: opts.sheetId`));
 ...
 * @returns {undefined} undefined
 */
Self.fillDataObjectsTabsHHMISheet = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `spreadsheetId`) === `undefined`)
    return cb(Error(`Missing required data: opts.spreadsheetId`));
 ...
 * @returns {undefined} undefined
 */
Self.fillAuthorsAndAffiliationHHMISheet = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `spreadsheetId`) === `undefined`)
    return cb(Error(`Missing required data: opts.spreadsheetId`));
  if (typeof _.get(opts, `sheetId`) === `undefined`) return cb(Error(`Missing required data: opts.sheetId`));
  ...
  let actions = [
    ...
    function (next) {
      let requests = [
        ...
        Self.buildRequestMergeCells(sheetId, 10, 11 + authors.length - 1, 1, 3),
        Self.buildRequestMergeCells(sheetId, 10, 11 + authors.length - 1, 3, 5),
    ...
    function (next) {
      ...
      for (let i = 0; i < authors.length; i++) {
        ...
        let formula = `=SI(C${rowIndex}, IFNA(IFS(ESTTEXTE(RECHERCHEV($A${rowIndex}, Authors!$A$2:$I, 9, FAUX)), RECHERCHEV($A${rowIndex}, Authors!$A$2:$I, 9, FAUX), ESTTEXTE(RECHERCHEV($A${rowIndex}, Authors!$B$2:$I, 8, FAUX)), RECHERCHEV($A${rowIndex}, Authors!$B$2:$I, 8, FAUX), EXACT(IFNA(RECHERCHEV(REGEXEXTRACT($A${rowIndex},"[A-Z]\\w+"), Authors!$C$2:$I, 7, FAUX), "N/A"), IFNA(RECHERCHEV(SUPPRESPACE(REGEXEXTRACT($A${rowIndex}, "[A-Z]\\w+\\s*$")), Authors!$D$2:$I, 6, FAUX), "N/A-2")), RECHERCHEV(REGEXEXTRACT($A${rowIndex}, "[A-Z]\\w+"), Authors!$C$2:$I, 7, FAUX), EXACT(IFNA(RECHERCHEV(REGEXEXTRACT($A${rowIndex},"[A-Z]\\w+"), Authors!$D$2:$I, 6, FAUX), "N/A"), IFNA(RECHERCHEV(SUPPRESPACE(REGEXEXTRACT($A${rowIndex}, "[A-Z]\\w+\\s*$")), Authors!$C$2:$I, 7, FAUX), "N/A-2")), RECHERCHEV(REGEXEXTRACT($A${rowIndex}, "[A-Z]\\w+"), Authors!$D$2:$I, 6, FAUX)), "Not Found"), "")`;
        let suggestedOrcids =
          authors[i].orcid.suggestedValues.length > 0
        ...
        data.push(Self.buildRequestUpdateCell(`'Authors and Affiliation'!D${rowIndex}`, [suggestedOrcids]));
      }
      return gSheets.spreadsheets.values.batchUpdate(
        {
          // The ID of the spreadsheet to update.
