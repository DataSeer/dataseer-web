...
Self.buildReport = function (opts, cb) {
  ...
  let finished = false;
  for (let i = 0; i < opts.data; i++) {
    if (finished) break;
    let data = opts.data[i];
    for (let j = 0; j < data.document.organizations.length; j++) {
      let organizationId = data.document.organizations[i];
      if (typeof folders[organizationId] !== `undefined`) {
        folder = folders[organizationId];
        finished = true;
        break;
  ...
  for (let i = 0; i < opts.data; i++) {
    ...
    for (let j = 0; j < data.document.organizations.length; j++) {
      let organizationId = data.document.organizations[i];
      if (typeof permission[organizationId] !== `undefined`) {
...
Self.buildDataForHHMISheets = function (data) {
  ...
  let article = { title: data.metadata.articleTitle, url: {} };
      ...
      for (let i = 0; i < articleInformation.length; i++) {
        ...
        data.push(
          Self.buildRequestUpdateCell(`'${sheetName}'!D${rowIndex}`, [
            `=LIEN_HYPERTEXTE("${articleInformation[i].publicationURL.url}", "${articleInformation[i].publicationURL.label}")`
          ])
        );
        data.push(
          Self.buildRequestUpdateCell(`'${sheetName}'!K${rowIndex}`, [
            `=LIEN_HYPERTEXTE("${articleInformation[i].preprintURL.url}", "${articleInformation[i].preprintURL.label}")`
          ])
        );
...
Self.fillDetailedReportHHMIPart = function (opts = {}, cb) {
  ...
  let batches = [
    // protocols
    {
      ...
      cells: [],
      params: {
        noDataRow: { min: 83, max: 85, display: conf.settings.displayNoDataSection[`HHMI`].protocols }
      },
      fn: function (batch) {
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 67, max: 67, insert: 67 },
      cells: [],
      params: {
        noDataRow: { min: 68, max: 70, display: conf.settings.displayNoDataSection[`HHMI`].materials }
      },
      fn: function (batch) {
        let link = `#gid=${sheetId}&range=B${batch.row.insert}`;
    ...
    // software
    {
      ...
      cells: [],
      params: {
        noDataRow: { min: 53, max: 55, display: conf.settings.displayNoDataSection[`HHMI`].software }
      },
      fn: function (batch) {
        let link = `#gid=${sheetId}&range=B${batch.row.insert}`;
        let label = `Software`;
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 37, max: 37, insert: 37 },
      cells: [],
      params: {
        noDataRow: { min: 38, max: 40, display: conf.settings.displayNoDataSection[`HHMI`].code }
      },
      fn: function (batch) {
        let link = `#gid=${sheetId}&range=B${batch.row.insert}`;
        let label = `Code`;
    ...
    // datasets
    {
      ...
      cells: [],
      params: {
        noDataRow: { min: 23, max: 25, display: conf.settings.displayNoDataSection[`HHMI`].datasets }
      },
      fn: function (batch) {
        let link = `#gid=${sheetId}&range=B${batch.row.insert}`;
        let label = `Datasets`;
...
Self.fillOpenScienceIndicatorSummaryHHMIPart = function (opts = {}, cb) {
  ...
  let actions = [
    ...
    function (next) {
      ...
      let data = [
        Self.buildRequestUpdateCell(`'${sheetName}'!G13`, [OSIS.datasets.newShared]),
        Self.buildRequestUpdateCell(`'${sheetName}'!H13`, [OSIS.datasets.newNotShared]),
        Self.buildRequestUpdateCell(`'${sheetName}'!I13`, [OSIS.datasets.reuseCited]),
        Self.buildRequestUpdateCell(`'${sheetName}'!J13`, [OSIS.datasets.reuseNotCited]),
        Self.buildRequestUpdateCell(`'${sheetName}'!G14`, [OSIS.code.newShared]),
        Self.buildRequestUpdateCell(`'${sheetName}'!H14`, [OSIS.code.newNotShared]),
        Self.buildRequestUpdateCell(`'${sheetName}'!I14`, [OSIS.code.reuseCited]),
        Self.buildRequestUpdateCell(`'${sheetName}'!J14`, [OSIS.code.reuseNotCited]),
        Self.buildRequestUpdateCell(`'${sheetName}'!G15`, [OSIS.software.newShared]),
        Self.buildRequestUpdateCell(`'${sheetName}'!H15`, [OSIS.software.newNotShared]),
        Self.buildRequestUpdateCell(`'${sheetName}'!I15`, [OSIS.software.reuseCited]),
        Self.buildRequestUpdateCell(`'${sheetName}'!J15`, [OSIS.software.reuseNotCited]),
        Self.buildRequestUpdateCell(`'${sheetName}'!G16`, [OSIS.labMaterials.newShared]),
        Self.buildRequestUpdateCell(`'${sheetName}'!H16`, [OSIS.labMaterials.newNotShared]),
        Self.buildRequestUpdateCell(`'${sheetName}'!I16`, [OSIS.labMaterials.reuseCited]),
        Self.buildRequestUpdateCell(`'${sheetName}'!J16`, [OSIS.labMaterials.reuseNotCited]),
        Self.buildRequestUpdateCell(`'${sheetName}'!G17`, [OSIS.protocols.newShared]),
        Self.buildRequestUpdateCell(`'${sheetName}'!H17`, [OSIS.protocols.newNotShared]),
        Self.buildRequestUpdateCell(`'${sheetName}'!I17`, [OSIS.protocols.reuseCited]),
        Self.buildRequestUpdateCell(`'${sheetName}'!J17`, [OSIS.protocols.reuseNotCited])
      ];
      return gSheets.spreadsheets.values.batchUpdate(
        {
          // The ID of the spreadsheet to update.
