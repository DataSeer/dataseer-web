...
Self.fillDetailedReportHHMIPart = function (opts = {}, cb) {
  ...
  let batches = [
    // protocols
    {
      ...
      row: { min: 83, max: 83, insert: 83 },
      cells: [],
      params: {
        noDataRow: { min: 84, max: 86, display: conf.settings.displayNoDataSection[`HHMI`].protocols }
      },
      fn: function (batch) {
    ...
    // lab materials
    {
      ...
      cells: [],
      params: {
        noDataRow: { min: 69, max: 71, display: conf.settings.displayNoDataSection[`HHMI`].materials }
      },
      fn: function (batch) {
        let link = `#gid=${sheetId}&range=B${batch.row.insert}`;
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 53, max: 53, insert: 53 },
      cells: [],
      params: {
        noDataRow: { min: 54, max: 56, display: conf.settings.displayNoDataSection[`HHMI`].software }
      },
      fn: function (batch) {
        let link = `#gid=${sheetId}&range=B${batch.row.insert}`;
        let label = `Software`;
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 38, max: 38, insert: 38 },
      cells: [],
      params: {
        noDataRow: { min: 39, max: 41, display: conf.settings.displayNoDataSection[`HHMI`].code }
      },
      fn: function (batch) {
        let link = `#gid=${sheetId}&range=B${batch.row.insert}`;
        let label = `Code`;
