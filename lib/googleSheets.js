...
Self.insertDataInDataSeerGenericSheets = function (opts = {}, cb) {
  ...
  let filteredCodes = [
    ...
    codes.filter(function (item) {
      return item.status === status.codes[`2`].label;
    }),
    // Verify citation
    codes.filter(function (item) {
      return item.status === status.codes[`1`].label;
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      return item.status === status.codes[`0`].label;
    })
  ];
...
Self.fillResearchOutputAvailabilityStatementGenericPart = function (opts = {}, cb) {
  ...
  let lines = {
    ...
    datasets: datasets.map(function (item) {
      return { name: item.name, URL: item.URL, identifier: item.PID, notes: item.comments };
    }),
    ...
    softwares: softwares.map(function (item) {
      return { name: item.name, URL: item.URL, identifier: item.RRID, notes: item.comments };
    }),
...
Self.fillDataObjectsTabsASAPSheet = function (opts = {}, cb) {
  ...
  let batches = [
    ...
    // software
    {
      ...
      column: { min: 0, max: 11 },
      params: {
        ignoreDeleteSheet: true
      },
      cells: [],
      data: [
        {
          merges: [],
          row: { min: 23, max: 28, insert: 26 },
          cells: [
            { column: `B`, property: `issue` },
            { column: `C`, property: `version` },
            { column: `D`, property: `name` },
    ...
    // code
    {
      ...
      params: {
        ignoreDeleteSheet:
          // ignore delete if there are softwares
          softwares.reduce(function (acc, category) {
        ...
        function (n) {
          ...
          if (typeof batch.copyPaste === `object`) {
            ...
            requests.push(
              Self.buildRequestInsertRows(
                batch.sheet.id,
                batch.copyPaste.row.end,
                batch.copyPaste.row.end + delta + 1,
                false
              )
            );
            requests.push(
          ...
          // if there is no data at all, delete head row(s)
          if (noData && !batch.params.ignoreDelete) {
            if (noData && !batch.params.ignoreDeleteRows)
              requests.push(Self.buildRequestDeleteRows(batch.sheet.id, batch.row.min - 1, batch.row.max));
            if (noData && !batch.params.ignoreDeleteSheet) requests.push(Self.buildRequestDeleteSheet(batch.sheet.id));
          }
          return gSheets.spreadsheets.batchUpdate(
            {
              // The ID of the spreadsheet to update.
