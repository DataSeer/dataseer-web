...
Self.insertDataInDataSeerGenericSheets = function (opts = {}, cb) {
  ...
  let filteredSoftwares = [
    ...
    softwares.filter(function (item) {
      return (
        (!item.reuse && item.version && !item.issues && !item.RRID && !item.suggestedEntity) ||
        (!item.reuse && !item.issues && item.DOI && !item.RRID && !item.suggestedEntity)
      );
    }),
    ...
    // Done - Cited correctly
    softwares.filter(function (item) {
      return item.version && !item.issues && item.DOI && item.RRID && !item.suggestedEntity;
    }),
  ...
  let filteredMaterials = [
    ...
    reagents.filter(function (item) {
      return !item.reuse && !item.issues && !item.RRID && !item.suggestedEntity;
    }),
