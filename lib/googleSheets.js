...
Self.buildReport = function (opts, cb) {
  ...
  return Self.createReportFile(
    ...
    function (err, id) {
      ...
      else if (kind === `MDAR Report`)
        return Self.insertDataInMDARReportSheets({ spreadsheetId: id, data: opts.data[0] }, function (err) {
          if (err) return cb(err);
          return cb(null, id);
        });
      else return cb(null, new Error(`Unavailable functionality`));
...
/**
 * This function will build data for MDAR Report
 * @param {object} data - Data available
 * @returns {object} Return data for MDAR ReportSheets
 */
Self.buildDataForMDARReportSheets = function (data) {
  let sections = {
    'Antibodies': data.reagents.filter(function (item) {
      return item.dataType === `lab materials` && item.subType === `antibodies`;
    }),
    'DNA and RNA sequences': data.reagents.filter(function (item) {
      return item.dataType === `lab materials` && (item.subType === `plasmid` || item.subType === `primers`);
    }),
    'Cell materials': data.reagents.filter(function (item) {
      return item.dataType === `lab materials` && item.subType === `cell line`;
    }),
    'Experimental animals': data.reagents.filter(function (item) {
      return item.dataType === `lab materials` && item.subType === `organism`;
    }),
    'Plants and microbes': data.reagents.filter(function (item) {
      return item.dataType === `lab materials` && item.subType === `plants and microbes`;
    }),
    'Human research participants': data.datasets.filter(function (item) {
      return item.dataType === `tabular data` && item.subType === `subject data table`;
    }),
    'Study Protocol': data.protocols.filter(function (item) {
      return item.dataType === `protocol` && item.subType === `antibodies`;
    }),
    'Laboratory Protocol': data.protocols.filter(function (item) {
      return (
        item.dataType === `protocol` &&
        (item.subType === `published protocol` ||
          (item.subType !== `antibodies` &&
            item.subType !== `human` &&
            item.subType !== `animal` &&
            item.subType !== `collection permit`))
      );
    }),
    'Ethics - Humans': data.protocols.filter(function (item) {
      return item.dataType === `protocol` && item.subType === `human`;
    }),
    'Ethics - Animal': data.protocols.filter(function (item) {
      return item.dataType === `protocol` && item.subType === `animal`;
    }),
    'Ethics - Filed and Sample': data.protocols.filter(function (item) {
      return item.dataType === `protocol` && item.subType === `collection permit`;
    }),
    'datasets': data.datasets.filter(function (item) {
      return !(item.dataType === `tabular data` && item.subType === `subject data table`);
    }),
    'code': data.code,
  ...
  let dataObjects = {
    'Antibodies': [
      sections[`Antibodies`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `reagents`, null, `4`)
        );
      }),
      sections[`Antibodies`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `reagents`, null, `3`)
        );
      }),
      sections[`Antibodies`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `reagents`, null, `2`)
        );
      }),
      sections[`Antibodies`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `reagents`, null, `1`)
        );
      }),
      sections[`Antibodies`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `reagents`, null, `0`)
        );
      })
    ],
    'DNA and RNA sequences': [
      sections[`DNA and RNA sequences`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `reagents`, null, `4`)
        );
      }),
    ...
    'Cell materials': [
      sections[`Cell materials`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `reagents`, null, `4`)
        );
      }),
    ...
    'Experimental animals': [
      sections[`Experimental animals`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `reagents`, null, `4`)
        );
      }),
    ...
    'Plants and microbes': [
      sections[`Plants and microbes`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `reagents`, null, `4`)
        );
      }),
    ...
    'Human research participants': [
      sections[`Human research participants`].filter(function (item) {
        return !item.reuse && !item.issues && !item.URL && !item.PID;
      }),
      sections[`Human research participants`].filter(function (item) {
        return item.reuse && !item.issues && !item.URL && !item.PID;
      }),
      ...
      sections[`Human research participants`].filter(function (item) {
        return !item.issues && (item.URL || item.PID);
      }),
      sections[`Human research participants`].filter(function (item) {
        return !item.issues && (item.URL || item.PID) && (item.qc || item.representativeImage);
      })
    ],
    'Study Protocol': [
      sections[`Study Protocol`].filter(function (item) {
        let subType = item.dataType === `other` ? `` : item.subType;
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) ===
          Self.getConfRuleKey(`MDAR Report`, `protocols`, subType, `0`)
        );
      })
    ],
    'Laboratory Protocol': [
      sections[`Laboratory Protocol`].filter(function (item) {
        let subType = item.dataType === `other` ? `` : item.subType;
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) ===
          Self.getConfRuleKey(`MDAR Report`, `protocols`, subType, `0`)
    ...
    'Ethics - Humans': [
      sections[`Ethics - Humans`].filter(function (item) {
        let subType = item.dataType === `other` ? `` : item.subType;
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) ===
          Self.getConfRuleKey(`MDAR Report`, `protocols`, subType, `0`)
    ...
    'Ethics - Animal': [
      sections[`Ethics - Animal`].filter(function (item) {
        let subType = item.dataType === `other` ? `` : item.subType;
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) ===
          Self.getConfRuleKey(`MDAR Report`, `protocols`, subType, `0`)
    ...
    'Ethics - Filed and Sample': [
      sections[`Ethics - Filed and Sample`].filter(function (item) {
        let subType = item.dataType === `other` ? `` : item.subType;
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) ===
          Self.getConfRuleKey(`MDAR Report`, `protocols`, subType, `0`)
    ...
    'datasets': [
      sections[`datasets`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `datasets`, null, `5`)
        );
      }),
      sections[`datasets`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `datasets`, null, `4`)
        );
      }),
      sections[`datasets`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `datasets`, null, `3`)
        );
      }),
      sections[`datasets`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `datasets`, null, `0`)
        );
      }),
      sections[`datasets`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) ===
            Self.getConfRuleKey(`MDAR Report`, `datasets`, null, `1`) ||
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `datasets`, null, `2`)
        );
      })
    ],
    'code': [
      sections[`code`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `code`, null, `2`)
        );
      }),
      sections[`code`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `code`, null, `3`)
        );
      }),
      sections[`code`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `code`, null, `4`)
        );
      }),
      sections[`code`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `code`, null, `0`)
        );
      }),
      sections[`code`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `code`, null, `1`)
        );
      })
    ],
    'software': [
      sections[`software`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `software`, null, `2`)
        );
      }),
      sections[`software`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `software`, null, `3`)
        );
      }),
      sections[`software`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `software`, null, `4`)
        );
      }),
      sections[`software`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `software`, null, `0`)
        );
      }),
      sections[`software`].filter(function (item) {
        return (
          Self.getRuleKey(item.rules[`MDAR Report`].rule) === Self.getConfRuleKey(`MDAR Report`, `software`, null, `1`)
        );
      })
    ]
  };
  ...
  let date = new Date();
  let MAS = data.metadata.MAS.content;
...
/**
 * This function will fill MDAR Report with given data and return the file ID (google file ID)
 * @param {object} opts - Options available
 * @param {string} opts.spreadsheetId - spreadsheetId
 * @param {object} opts.data - Data available
 * @param {function} cb - Callback function(err, res) (err: error process OR null, res: google file ID OR undefined)
 * @returns {undefined} undefined
 */
Self.insertDataInMDARReportSheets = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `spreadsheetId`) === `undefined`)
    return cb(Error(`Missing required data: opts.spreadsheetId`));
  let spreadsheetId = _.get(opts, `spreadsheetId`);
  let data = _.get(opts, `data`, {});
  return Self.getSheets({ spreadsheetId: spreadsheetId }, function (err, sheets) {
    let sheetId = Self.getSheetId(sheets, `Report`);
    if (sheetId instanceof Error) return cb(sheetId);
    return Self.fillReportMDARReportSheet(
      {
        spreadsheetId: spreadsheetId,
        sheetId: sheetId,
        sheetName: `Report`,
        data: Self.buildDataForMDARReportSheets(data)
      },
      function (err, res) {
        return cb(err);
      }
 ...
 * @returns {undefined} undefined
 */
Self.fillReportMDARReportSheet = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `spreadsheetId`) === `undefined`)
    return cb(Error(`Missing required data: opts.spreadsheetId`));
  if (typeof _.get(opts, `sheetId`) === `undefined`) return cb(Error(`Missing required data: opts.sheetId`));
  ...
  let actions = [
    ...
    function (next) {
      ...
      return Self.fillDataObjectsMDARReportPart(
        {
          spreadsheetId: spreadsheetId,
          sheetId: sheetId,
          sheetName: sheetName,
    ...
    function (next) {
      ...
      // URL
      // DAS
      return Self.fillSummaryMDARReportPart(
        {
          spreadsheetId: spreadsheetId,
          sheetId: sheetId,
          sheetName: sheetName,
 ...
 * @returns {undefined} undefined
 */
Self.fillDataObjectsMDARReportPart = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `spreadsheetId`) === `undefined`)
    return cb(Error(`Missing required data: opts.spreadsheetId`));
  if (typeof _.get(opts, `sheetId`) === `undefined`) return cb(Error(`Missing required data: opts.sheetId`));
  ...
  let batches = [
    // software (Software availability)
    {
      ...
      row: { min: 269, max: 269, insert: 269 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: {
          min: 270,
          max: 271,
          display: conf.settings.displayNoDataSection[`MDAR Report`][`software`]
        }
      },
      data: [
        {
          merges: [],
          row: { min: 272, max: 276, insert: 274 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
            { column: `D`, property: `RRID` },
            { column: `E`, property: `URL` },
            { column: `F`, property: `notes` }
          ],
          data: dataObjects[`software`][0].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
              ...
              sentences: item.sentences
                ...
                .join(` `),
              articleSubsection: item.headSentence,
              RRID: item.RRID,
              URL: item.URL,
              notes: item.comments
            };
        ...
        {
          merges: [],
          row: { min: 277, max: 281, insert: 280 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`software`][1].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 282, max: 286, insert: 285 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`software`][2].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 287, max: 291, insert: 290 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`software`][3].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 292, max: 298, insert: 295 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`software`][4].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 241, max: 241, insert: 241 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: {
          min: 242,
          max: 243,
          display: conf.settings.displayNoDataSection[`MDAR Report`][`code`]
        }
      },
      data: [
        {
          merges: [],
          row: { min: 244, max: 248, insert: 247 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
            { column: `D`, property: `URL` },
            { column: `E`, property: `DOI` },
            { column: `F`, property: `notes` }
          ],
          data: dataObjects[`code`][0].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 249, max: 253, insert: 252 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`code`][1].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 254, max: 258, insert: 257 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`code`][2].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 259, max: 263, insert: 262 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`code`][3].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 264, max: 268, insert: 267 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`code`][4].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 213, max: 213, insert: 213 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: {
          min: 214,
          max: 215,
          display: conf.settings.displayNoDataSection[`MDAR Report`][`datasets`]
        }
      },
      data: [
        {
          merges: [],
          row: { min: 216, max: 220, insert: 219 },
          cells: [
            { column: `A`, property: `dataType` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
            { column: `D`, property: `URL` },
            { column: `E`, property: `PID` },
            { column: `F`, property: `notes` }
          ],
          data: dataObjects[`datasets`][0].map(function (item) {
            return {
        ...
        {
          merges: [],
          row: { min: 221, max: 225, insert: 224 },
          cells: [
            { column: `A`, property: `dataType` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`datasets`][1].map(function (item) {
            return {
              dataType: item.type.url ? `=LIEN_HYPERTEXTE("${item.type.url}";"${item.type.label}")` : item.type.label,
              sentences: item.sentences
                .map(function (s) {
        ...
        {
          merges: [],
          row: { min: 226, max: 230, insert: 229 },
          cells: [
            { column: `A`, property: `dataType` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`datasets`][2].map(function (item) {
            return {
              dataType: item.type.url ? `=LIEN_HYPERTEXTE("${item.type.url}";"${item.type.label}")` : item.type.label,
              sentences: item.sentences
                .map(function (s) {
        ...
        {
          merges: [],
          row: { min: 231, max: 235, insert: 234 },
          cells: [
            { column: `A`, property: `dataType` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`datasets`][3].map(function (item) {
            return {
              dataType: item.type.url ? `=LIEN_HYPERTEXTE("${item.type.url}";"${item.type.label}")` : item.type.label,
              sentences: item.sentences
                .map(function (s) {
        ...
        {
          merges: [],
          row: { min: 236, max: 240, insert: 239 },
          cells: [
            { column: `A`, property: `dataType` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`datasets`][4].map(function (item) {
            return {
              dataType: item.type.url ? `=LIEN_HYPERTEXTE("${item.type.url}";"${item.type.label}")` : item.type.label,
              sentences: item.sentences
                .map(function (s) {
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 201, max: 201, insert: 201 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: {
      ...
      data: [
        {
          ...
          row: { min: 207, max: 207, insert: 207 },
          cells: [
            { column: `C`, property: `details` },
            { column: `F`, property: `articleSubsection` }
          ],
          params: {
            noData: { display: false }
          },
          data: dataObjects[`Ethics - Humans`][0].map(function (item) {
            return {
              details: item.sentences
                .map(function (s) {
                  return s.text;
                })
                .join(` `),
        ...
        {
          merges: [{ begin: 2, end: 5 }],
          row: { min: 208, max: 208, insert: 208 },
          cells: [
            { column: `C`, property: `details` },
            { column: `F`, property: `articleSubsection` }
          ],
          ...
          data: dataObjects[`Ethics - Animal`][0].map(function (item) {
            return {
              details: item.sentences
                .map(function (s) {
                  return s.text;
        ...
        {
          merges: [{ begin: 2, end: 5 }],
          row: { min: 209, max: 209, insert: 209 },
          cells: [
            { column: `C`, property: `details` },
            { column: `F`, property: `articleSubsection` }
          ],
          ...
          data: dataObjects[`Ethics - Filed and Sample`][0].map(function (item) {
            return {
              details: item.sentences
                .map(function (s) {
                  return s.text;
    ...
    // Ethics - No action required section
    // hack to remove this section if there is no data
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 204, max: 206, insert: 204 },
      cells: [],
      params: {
        hideRowNumber: true
      },
      data: [
        {
          row: { min: 205, max: 205, insert: 205 },
          cells: [{ column: `B`, property: `empty` }],
          params: {
            noData: { display: false }
          },
          data:
            dataObjects[`Ethics - Humans`][0].length +
              dataObjects[`Ethics - Animal`][0].length +
              dataObjects[`Ethics - Filed and Sample`][0] >
            0
              ? [{ 'empty': `` }]
              : []
        }
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 193, max: 193, insert: 193 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: {
          min: 194,
          max: 195,
          display: conf.settings.displayNoDataSection[`MDAR Report`][`Laboratory Protocol`]
        }
      },
      data: [
        {
          merges: [{ begin: 3, end: 6 }],
          row: { min: 196, max: 200, insert: 199 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
            { column: `D`, property: `DOI` }
          ],
          data: dataObjects[`Laboratory Protocol`][0].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 185, max: 185, insert: 185 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: { min: 186, max: 187, display: conf.settings.displayNoDataSection[`MDAR Report`][`Study Protocol`] }
      },
      data: [
        {
          merges: [{ begin: 3, end: 6 }],
          row: { min: 188, max: 192, insert: 191 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Study Protocol`][0].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 155, max: 155, insert: 155 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: {
          min: 156,
          max: 157,
          display: conf.settings.displayNoDataSection[`MDAR Report`][`Human research participants`]
        }
      },
      data: [
        {
          merges: [],
          row: { min: 158, max: 162, insert: 161 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Human research participants`][0].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 163, max: 167, insert: 166 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Human research participants`][1].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 168, max: 172, insert: 171 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Human research participants`][2].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 173, max: 177, insert: 176 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Human research participants`][3].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 178, max: 183, insert: 181 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Human research participants`][4].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 127, max: 127, insert: 127 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: {
          min: 128,
          max: 129,
          display: conf.settings.displayNoDataSection[`MDAR Report`][`Plants and microbes`]
        }
      },
      data: [
        {
          merges: [],
          row: { min: 130, max: 134, insert: 133 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
            { column: `D`, property: `source` },
            { column: `E`, property: `identifier` },
            { column: `F`, property: `notes` }
          ],
          data: dataObjects[`Plants and microbes`][0].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
              ...
              articleSubsection: item.headSentence,
              source: item.source,
              identifier: `${item.catalogNumber ? `${item.catalogNumber}\n` : ``}${item.RRID ? `${item.RRID}\n` : ``}`,
              notes: item.comments
            };
          })
        },
        {
          merges: [],
          row: { min: 135, max: 139, insert: 138 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Plants and microbes`][1].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 140, max: 144, insert: 143 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Plants and microbes`][2].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 145, max: 149, insert: 148 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Plants and microbes`][3].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 150, max: 154, insert: 153 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Plants and microbes`][4].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
    ...
    // Experimental animals
    {
      ...
      params: {
        ...
        noDataRow: {
          min: 100,
          max: 101,
          display: conf.settings.displayNoDataSection[`MDAR Report`][`Experimental animals`]
        }
      },
      data: [
        {
          merges: [],
          row: { min: 102, max: 106, insert: 105 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Experimental animals`][0].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 107, max: 111, insert: 110 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Experimental animals`][1].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 112, max: 116, insert: 115 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Experimental animals`][2].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 117, max: 121, insert: 120 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Experimental animals`][3].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        {
          merges: [],
          row: { min: 122, max: 126, insert: 125 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Experimental animals`][4].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 71, max: 71, insert: 71 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: { min: 72, max: 73, display: conf.settings.displayNoDataSection[`MDAR Report`][`Cell materials`] }
      },
      data: [
        {
          merges: [],
          row: { min: 74, max: 78, insert: 77 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Cell materials`][0].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`Cell materials`][1].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`Cell materials`][2].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`Cell materials`][3].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`Cell materials`][4].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
    ...
    // DNA and RNA sequences
    {
      ...
      params: {
        ...
        noDataRow: {
          min: 44,
          max: 45,
          display: conf.settings.displayNoDataSection[`MDAR Report`][`DNA and RNA sequences`]
        }
      },
      data: [
        {
          merges: [],
          row: { min: 46, max: 50, insert: 49 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`DNA and RNA sequences`][0].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`DNA and RNA sequences`][1].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`DNA and RNA sequences`][2].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`DNA and RNA sequences`][3].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`DNA and RNA sequences`][4].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 15, max: 15, insert: 15 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: { min: 16, max: 17, display: conf.settings.displayNoDataSection[`MDAR Report`][`Antibodies`] }
      },
      data: [
        {
          merges: [],
          row: { min: 18, max: 22, insert: 21 },
          cells: [
            { column: `A`, property: `name` },
            { column: `B`, property: `sentences` },
            { column: `C`, property: `articleSubsection` },
          ...
          data: dataObjects[`Antibodies`][0].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`Antibodies`][1].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`Antibodies`][2].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`Antibodies`][3].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
        ...
        },
        {
          ...
          data: dataObjects[`Antibodies`][4].map(function (item) {
            return {
              name: conf.settings.dataObjectsLinks[`MDAR Report`]
                ? Self.buildDataObjectLink(documentId, documentToken, item._id.toString(), item.name)
                : item.name,
    ...
    {
      sheet: { id: sheetId, name: sheetName },
      row: { min: 6, max: 6, insert: 6 },
      cells: [],
      params: {
        hideRowNumber: true,
        noDataRow: { min: 7, max: 8, display: conf.settings.displayNoDataSection[`MDAR Report`][`MAS`] }
      },
      data: [
        {
          merges: [],
          row: { min: 9, max: 11, insert: 10 },
          cells: [{ column: `C`, property: `MAS` }],
          data: MAS === `` ? [{ 'MAS': MAS }] : []
        },
        {
          merges: [],
          row: { min: 12, max: 14, insert: 13 },
          cells: [{ column: `C`, property: `MAS` }],
          data: MAS !== `` && MAS !== `n/a` ? [{ 'MAS': MAS }] : []
        }
      ]
    }
  ];
    ...
    function (batch, _next) {
      ...
      let actions = [
        ...
        function (n) {
          ...
          if (
            (!noData && batch.params.noDataRow && batch.params.noDataRow.min && batch.params.noDataRow.max) ||
            batch.params?.noDataRow?.display === false
          ) {
            requests.push(
          ...
          for (let i = batch.data.length - 1; i >= 0; i--) {
            ...
            // delete row(s) if necessary
              ...
              if (!batch.data[i]?.params?.noData?.display)
          ...
          return gSheets.spreadsheets.batchUpdate(
            ...
            function (err, res) {
              setTimeout(function () {
                ...
                return n(err);
              }, conf.settings.googleAPI[`MDAR Report`].timeout);
            }
          );
        },
 ...
 * @returns {undefined} undefined
 */
Self.fillSummaryMDARReportPart = function (opts = {}, cb) {
  // Check all required data
  if (typeof _.get(opts, `spreadsheetId`) === `undefined`)
    return cb(Error(`Missing required data: opts.spreadsheetId`));
  if (typeof _.get(opts, `sheetId`) === `undefined`) return cb(Error(`Missing required data: opts.sheetId`));
  ...
  let actions = [
    ...
    function (next) {
      let date = new Date();
      let data = [
        Self.buildRequestUpdateCell(`'${sheetName}'!B2`, [article.title]),
        Self.buildRequestUpdateCell(`'${sheetName}'!B3`, [
          `=LIEN_HYPERTEXTE("${article.URL.url}";"${article.URL.url ? article.URL.url : article.URL.label}")`
        ]),
        Self.buildRequestUpdateCell(`'${sheetName}'!B4`, [DAS])
      ];
      return gSheets.spreadsheets.values.batchUpdate(
        {
          // The ID of the spreadsheet to update.
    ...
    function (batch, _next) {
      ...
      let actions = [
        ...
        function (n) {
          ...
          if (
            (!noData && batch.params.noDataRow && batch.params.noDataRow.min && batch.params.noDataRow.max) ||
            !batch.params.noDataRow.display
          ) {
            requests.push(
              Self.buildRequestDeleteRows(batch.sheet.id, batch.params.noDataRow.min - 1, batch.params.noDataRow.max)
            );
        ...
        return Self.fillSummaryASAPSheet(
          {
            ...
            data: {
              ...
              krtFileLink: metadata.krtFileLink,
  ...
  let actions = [
    ...
    function (next) {
      ...
      if (cellLines.length > 1) {
        otherInformation.push(Self.buildRequestInsertRows(sheetId, 50, 50 + cellLines.length - 1, true));
        otherInformation.push(
          Self.buildRequestUpdateBorders(sheetId, 50, 50 + cellLines.length - 1, 3, 4, GRAY_BORDERS)
        );
      }
      if (animalModels.length > 1) {
        otherInformation.push(Self.buildRequestInsertRows(sheetId, 49, 49 + animalModels.length - 1, true));
        otherInformation.push(
          Self.buildRequestUpdateBorders(sheetId, 49, 49 + animalModels.length - 1, 3, 4, GRAY_BORDERS)
        );
      }
      // Headers of "Other Information" section
      otherInformation.push(Self.buildRequestUpdateBorders(sheetId, 48, 49, 0, 4, GRAY_BORDERS));
      ...
      otherInformation.push(Self.buildRequestUpdateBorders(sheetId, 50 + offset, 51 + offset, 0, 4, GRAY_BORDERS));
      let researchOutputAvailabilityStatement = [];
      if (lines.protocols.length > 1)
        researchOutputAvailabilityStatement.push(
          Self.buildRequestInsertRows(sheetId, 45, 45 + lines.protocols.length - 1, true)
        );
      if (lines.materials.length > 1)
        researchOutputAvailabilityStatement.push(
          Self.buildRequestInsertRows(sheetId, 44, 44 + lines.materials.length - 1, true)
        );
      if (lines.codeAndSoftware.length > 1)
        researchOutputAvailabilityStatement.push(
          Self.buildRequestInsertRows(sheetId, 43, 43 + lines.codeAndSoftware.length - 1, true)
        );
      if (lines.datasets.length > 1)
        researchOutputAvailabilityStatement.push(
          Self.buildRequestInsertRows(sheetId, 42, 42 + lines.datasets.length - 1, true)
        );
      researchOutputAvailabilityStatement.push(
        Self.buildRequestUpdateBorders(sheetId, 41, 41 + lines.all.length, 6, 9, GRAY_BORDERS)
      );
      researchOutputAvailabilityStatement.push(
        Self.buildRequestUpdateBorders(sheetId, 41, 41 + lines.all.length, 5, 6, BLACK_BORDERS)
      );
      let deleteOffsetOfSections = [
        Self.buildRequestDeleteRange(sheetId, 39, 41 + lines.all.length, 0, 4, `ROWS`) // Other Information section
      ];
    ...
    function (next) {
      let data = [];
      let offset = 42;
      // Preprint/Publication DOI
      data.push(
        Self.buildRequestUpdateCell(`'Summary Sheet'!I41`, [
      ...
      offset = 43; // Because this section is now on the left of ROAS section
          ...
          data: protocols[0].map(function (item) {
            return {
              ...
              sentences: item.sentences
                ...
                .join(` `)
                .replace(/\n+/gim, conf.settings.sentencesCarriageReturnReplace[`ASAP`])
            };
          })
        },
        {
      ...
      data: [
        {
          ...
          cells: [
            ...
            { column: `L`, property: `hasReadMe` },
            { column: `M`, property: `datatype` }
          ],
          // Deposit to appropriate repository and cite PID in text
          data: datasets[0].map(function (item) {
            return {
    ...
    function (batch, _next) {
      ...
      let actions = [
        ...
        function (n) {
          ...
          if (!noData && batch.params.noDataRow && batch.params.noDataRow.min && batch.params.noDataRow.max) {
            requests.push(
              Self.buildRequestDeleteRows(batch.sheet.id, batch.params.noDataRow.min - 1, batch.params.noDataRow.max)
            );
            offset -= batch.params.noDataRow.max - batch.params.noDataRow.min + 1;
...
Self.fillSummaryASAPSheet = function (opts = {}, cb) {
  ...
  let krtFileLink = _.get(data, `krtFileLink`, {});
  ...
  let actions = [
    ...
    function (next) {
      ...
      let data = [
        ...
        Self.buildRequestUpdateCell(`'Summary Sheet'!G9`, [
          krtFileLink && krtFileLink.url ? `=LIEN_HYPERTEXTE("${krtFileLink.url}";"${krtFileLink.label}")` : ``
        ]),
          ...
          data: protocols[0].map(function (item) {
            return {
              ...
              sentences: item.sentences
                ...
                .join(` `)
                .replace(/\n+/gim, conf.settings.sentencesCarriageReturnReplace[`ASAP-PPMI`])
            };
          })
        },
        {
          ...
          data: protocols[0].map(function (item) {
            return {
              ...
              sentences: item.sentences
                ...
                .join(` `)
                .replace(/\n+/gim, conf.settings.sentencesCarriageReturnReplace[`ASAP-GP2`])
            };
          })
        },
        {
