        ...
        return Self.fillResearchOutputAvailabilityStatementGenericPart(
          {
            ...
            data: {
              authors: metadata.authors,
              datasets: filteredDataObject.datasets.filtered[3],
              codes: filteredDataObject.codes.filtered[2],
              softwares: filteredDataObject.softwares.filtered[3].concat(filteredDataObject.softwares.filtered[4]),
              materials: filteredDataObject.materials.filtered[3],
              protocols: filteredDataObject.protocols.filtered[3]
            }
          },
          function (err, res) {
            return next(err);
...
Self.fillResearchOutputAvailabilityStatementGenericPart = function (opts = {}, cb) {
  ...
  let actions = [
    ...
    function (next) {
      let requests = [];
      if (orcids.length) requests.push(Self.buildRequestInsertRows(sheetId, 13, 13 + orcids.length - 1, true));
      if (protocols.length) requests.push(Self.buildRequestInsertRows(sheetId, 12, 12 + protocols.length - 1, true));
      if (materials.length) requests.push(Self.buildRequestInsertRows(sheetId, 11, 11 + materials.length - 1, true));
      if (softwares.length) requests.push(Self.buildRequestInsertRows(sheetId, 10, 10 + softwares.length - 1, true));
      if (codes.length) requests.push(Self.buildRequestInsertRows(sheetId, 9, 9 + codes.length - 1, true));
      if (datasets.length) requests.push(Self.buildRequestInsertRows(sheetId, 8, 8 + datasets.length - 1, true));
      if (lines.length) {
        requests.push(Self.buildRequestMergeCells(sheetId, 8, 8 + lines.length - 1, 1, 5));
        requests.push(Self.buildRequestMergeCells(sheetId, 8, 8 + lines.length - 1, 6, 9));
        requests.push(Self.buildRequestMergeCells(sheetId, 8, 8 + lines.length - 1, 9, 12));
        requests.push(Self.buildRequestUpdateBorders(sheetId, 8, 8 + lines.length - 1, 2, 12));
      }
      if (requests.length <= 0) return cb();
      return gSheets.spreadsheets.batchUpdate(
        {
          // The ID of the spreadsheet to update.
          spreadsheetId: spreadsheetId,
    ...
    function (next) {
      ...
      for (let i = 0; i < lines.length; i++) {
        let rowIndex = 8 + i;
        data.push(Self.buildRequestUpdateCell(`'Detailed Report'!F${rowIndex}`, [lines[i].name]));
        data.push(Self.buildRequestUpdateCell(`'Detailed Report'!G${rowIndex}`, [lines[i].URL]));
        data.push(Self.buildRequestUpdateCell(`'Detailed Report'!J${rowIndex}`, [lines[i].notes]));
      }
      return gSheets.spreadsheets.values.batchUpdate(
        {
          // The ID of the spreadsheet to update.
