...
const status = require(`../resources/rules/status.js`);
...
Self.insertDataInDataSeerGenericSheets = function (opts = {}, cb) {
  ...
  let filteredCodes = [
    ...
    codes.filter(function (item) {
      ...
      return item.status === status.codes[subType][`2`].label;
    }),
    // Verify citation
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === status.codes[subType][`1`].label;
    }),
    // Done - Cited correctly
    codes.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === status.codes[subType][`0`].label;
    })
  ];
  let filteredSoftwares = [
    ...
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === status.softwares[subType][`2`].label;
    }),
    // Verify citation
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === status.softwares[subType][`1`].label;
    }),
    // None - Citation meets minimum standards
    softwares.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === status.softwares[subType][`0`].label;
    })
  ];
  let filteredMaterials = [
    // Assign RRID and cite in text
    reagents.filter(function (item) {
      return item.status === status.reagents[`3`].label;
    }),
    // Check for an existing RRID
    reagents.filter(function (item) {
      return item.status === status.reagents[`2`].label;
    }),
    // Verify citation
    reagents.filter(function (item) {
      return item.status === status.reagents[`1`].label;
    }),
    // Done - Cited correctly
    reagents.filter(function (item) {
      return item.status === status.reagents[`0`].label;
    })
  ];
  let filteredProtocols = [
    ...
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === status.protocols[subType][`2`].label;
    }),
    // Include missing information in citation
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === status.protocols[subType][`3`].label;
    }),
    // Verify citation
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === status.protocols[subType][`1`].label;
    }),
    // Done - Cited correctly
    protocols.filter(function (item) {
      let subType = item.dataType === `other` ? `` : item.subType;
      return item.status === status.protocols[subType][`0`].label;
    })
  ];
...
Self.fillResearchOutputAvailabilityStatementDataSeerGenericPart = function (opts = {}, cb) {
  ...
  for (let i = 0; i < authors.length; i++) {
    ...
    if (orcidsFromTEI.length > 0) orcids.push(orcidsFromTEI);
  }
  let lines = {
    orcids: orcids.map(function (item) {
      return { name: item, URL: ``, identifier: ``, notes: `` };
    }),
  ...
  let actions = [
    ...
    function (next) {
      let requests = [
        Self.buildRequestInsertRows(sheetId, 14, 14 + lines.orcids.length - 1, true),
        Self.buildRequestInsertRows(sheetId, 13, 13 + lines.protocols.length - 1, true),
        Self.buildRequestInsertRows(sheetId, 12, 12 + lines.materials.length - 1, true),
        Self.buildRequestInsertRows(sheetId, 11, 11 + lines.softwares.length - 1, true),
        Self.buildRequestInsertRows(sheetId, 10, 10 + lines.codes.length - 1, true),
        Self.buildRequestInsertRows(sheetId, 9, 9 + lines.datasets.length - 1, true),
        Self.buildRequestMergeCells(sheetId, 9, 9 + lines.all.length - 1, 1, 5),
        Self.buildRequestMergeCells(sheetId, 9, 9 + lines.all.length - 1, 7, 9),
        Self.buildRequestMergeCells(sheetId, 9, 9 + lines.all.length - 1, 9, 12),
        Self.buildRequestUpdateBorders(sheetId, 9, 9 + lines.all.length - 1, 1, 12)
      ];
      return gSheets.spreadsheets.batchUpdate(
        {
          // The ID of the spreadsheet to update.
    ...
    function (next) {
      ...
      for (let i = 0; i < lines.all.length; i++) {
        ...
        data.push(Self.buildRequestUpdateCell(`'Detailed Report'!G${rowIndex}`, [lines.all[i].identifier]));
...
Self.fillAuthorsAndAffiliationDataSeerGenericPart = function (opts = {}, cb) {
  ...
  let actions = [
    ...
    function (next) {
      let requests = [
        // Lab Materials
        Self.buildRequestInsertRows(sheetId, begin, begin + 1 + authors.length - 1, true),
        Self.buildRequestMergeCells(sheetId, begin, begin + 1 + authors.length - 1, 2, 6),
        Self.buildRequestMergeCells(sheetId, begin, begin + 1 + authors.length - 1, 6, 9),
        Self.buildRequestMergeCells(sheetId, begin, begin + 1 + authors.length - 1, 9, 12),
        Self.buildRequestUpdateBorders(sheetId, begin, begin + 1 + authors.length - 1, 2, 12)
      ];
      return gSheets.spreadsheets.batchUpdate(
        {
          // The ID of the spreadsheet to update.
      ...
      for (let i = 0; i < authors.length; i++) {
        ...
        data.push(
          Self.buildRequestUpdateCell(`'Detailed Report'!J${rowIndex}`, [
            `=SIERREUR(RECHERCHEV($A${begin + i},Authors!$A$2:$I,9,FALSE), "Not found")`
          ])
        );
      }
      return gSheets.spreadsheets.values.batchUpdate(
      ...
      if (maxOffset > 0) {
        manageRangeFigureSection.push(
          Self.buildRequestInsertRange(
            ...
            40 + otherInformationOffset + maxOffset,
            0,
            4,
            `ROWS`
          )
      ...
      for (let i = 0; i < authors.length; i++) {
        ...
        data.push(
          Self.buildRequestUpdateCell(`'Authors and Affiliation'!E${rowIndex}`, [
            `=SI(C${10 + i}, IFNA(IFS(ESTTEXTE(RECHERCHEV(REGEXEXTRACT($A${
              10 + i
            }, "([à-üÀ-ÜA-Za-z-\\s]+)[\\s0-9,#*&^]*"), Authors!$A$2:$I,9,FAUX)), RECHERCHEV(REGEXEXTRACT($A${
              10 + i
            }, "([à-üÀ-ÜA-Za-z-\\s]+)[\\s0-9,#*&^]*"), Authors!$A$2:$I,9,FAUX), ESTTEXTE(RECHERCHEV(REGEXEXTRACT($A${
              10 + i
            }, "([à-üÀ-ÜA-Za-z-\\s]+)[\\s0-9,#*&^]*"), Authors!$B$2:$I,8,FAUX)), RECHERCHEV(REGEXEXTRACT($A${
              10 + i
            },"([à-üÀ-ÜA-Za-z-\\s]+)[\\s0-9,#*&^]*"), Authors!$B$2:$I,8,FAUX), ESTTEXTE(RECHERCHEV(CONCATENER(REGEXEXTRACT($A${
              10 + i
            }, "[à-üÀ-ÜA-Za-z-]+"), " ", REGEXEXTRACT($A${
              10 + i
            },"([à-üÀ-ÜA-Za-z-]+)[\\s0-9,#*&^]*$")), Authors!$A$2:$I,9,FAUX)), RECHERCHEV(CONCATENER(REGEXEXTRACT($A${
              10 + i
            },"[à-üÀ-ÜA-Za-z-]+"), " ", REGEXEXTRACT($A${
              10 + i
            },"([à-üÀ-ÜA-Za-z-]+)[\\s0-9,#*&^]*$")), Authors!$A$2:$I,9,FAUX), ESTTEXTE(RECHERCHEV(CONCATENER(REGEXEXTRACT($A${
              10 + i
            },"[à-üÀ-ÜA-Za-z-]+"), " ", REGEXEXTRACT($A${
              10 + i
            },"([à-üÀ-ÜA-Za-z-]+)[\\s0-9,#*&^]*$")), Authors!$B$2:$I,8,FAUX)), RECHERCHEV(CONCATENER(REGEXEXTRACT($A${
              10 + i
            },"[à-üÀ-ÜA-Za-z-]+"), " ", REGEXEXTRACT($A${
              10 + i
            },"([à-üÀ-ÜA-Za-z-]+)[\\s0-9,#*&^]*$")), Authors!$B$2:$I,8,FAUX)), "Not Found"), "")`
          ])
        );
      }
      return gSheets.spreadsheets.values.batchUpdate(
