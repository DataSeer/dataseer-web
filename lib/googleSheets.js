...
Self.insertDataInASAPSheets = function (opts = {}, cb) {
  ...
  let filteredSoftwares = [
    ...
    softwares.filter(function (item) {
      return (
        (item.reuse && !item.RRID && !item.DOI && !item.issue) || (item.reuse && !item.RRID && !item.qc && !item.issue)
      );
    }),
    softwares.filter(function (item) {
      return item.issue;
    ...
    softwares.filter(function (item) {
      return (
        ...
        (item.reuse && !item.RRID && item.version && !item.issue && item.DOI)
      );
    }),
    softwares.filter(function (item) {
      return item.RRID && item.version && !item.issue && item.DOI;
    })
  ];
  ...
  let filteredProtocols = [
    ...
    protocols.filter(function (item) {
      return !item.issue && (item.DOI === `Manufacturer's Instructions` || Params.isURL(item.DOI));
    })
  ];
  ...
  let actions = [
    ...
    function (next) {
      ...
      let data = [
        ...
        Self.buildRequestUpdateCell(`'Summary Sheet'!B29`, [materials.new.identified]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!C29`, [materials.new.shared]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!B30`, [materials.reuse.identified]),
        Self.buildRequestUpdateCell(`'Summary Sheet'!C30`, [materials.reuse.shared]),
...
Self.fillComplianceReportASAPSheet = function (opts = {}, cb) {
  ...
  let protocols = _.get(data, `protocols`, []);
  let codeAndSoftware = _.get(data, `codeAndSoftware`, []);
  let materials = _.get(data, `labMaterials`, []);
  let datasets = _.get(data, `datasets`, []);
  let sortedDatasets = {};
  // Regroup datasets by datatype
  for (let i = 0; i < datasets.length; i++) {
  ...
  let actions = [
    ...
    function (next) {
      let requests = [
        // Lab Materials
        Self.buildRequestInsertRows(sheetId, 26, 27 + materials.length - 1, true),
        Self.buildRequestUpdateBorders(sheetId, 26, 27 + materials.length - 1, 0, 9),
        // Code & Software
    ...
    function (next) {
      ...
      // Lab Materails
      for (let i = 0; i < batches.materials.length; i++) {
        let item = batches.materials[i];
        let rowIndex = rowOffset + i;
        data.push(Self.buildRequestUpdateCell(`'Compliance Report'!A${rowIndex}`, [i + 1]));
        data.push(Self.buildRequestUpdateCell(`'Compliance Report'!B${rowIndex}`, [item.reuse]));
        data.push(Self.buildRequestUpdateCell(`'Compliance Report'!C${rowIndex}`, [item.isValid ? `No` : `Yes`]));
      ...
      // data.push(
      //   Self.buildRequestUpdateCell(`'Compliance Report'!A${rowOffset + batches.materials.length + 1}`, [
      //     `=NB.SI(A${rowOffset}:A${rowOffset + batches.materials.length};">0")`
      //   ])
      // );
      data.push(
        Self.buildRequestUpdateCell(`'Compliance Report'!B${rowOffset + batches.materials.length + 1}`, [
          `=NB.SI(B${rowOffset}:B${rowOffset + batches.materials.length};VRAI)`
        ])
      );
      return gSheets.spreadsheets.values.batchUpdate(
        {
