...
const jsonCustomList = require(`../resources/jsonCustomSoftwareList.json`);
...
Self.refreshCustomList = function (opts = {}, cb) {
  ...
  if (typeof process.env.LOCAL_CUSTOM_SOFTWARE_LIST !== `undefined` || opts.local)
            ...
            if (score >= conf.settings.extractFromText.minScore) {
              if (typeof match[item.index] === `undefined`) {
                match[item.index] = {
                  ...
                  software: Self.getItemByIndex(item.index),
                  maxScore: score
                };
              } else {
