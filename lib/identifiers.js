...
Self.refreshCustomList = function (opts = {}, cb) {
  ...
  if (typeof process.env.LOCAL_CUSTOM_IDENTIFIERS_LIST !== `undefined` || opts.local)
...
Self.findFromCustomList = function (name, opts = {}) {
  ...
  let sanitizedName = accents
    .remove(name.replace(/\s+/gim, ` `))
    .replace(/^(?:(?:http)?(?:s)?(?:\:\/\/)?(?:dx\.|www\.)?doi\.org\/)?/gim, ``); // Remove DOI prefix
  ...
  if (contain)
    for (let key in Self.customList.mappings.name) {
      if (key.indexOf(sanitizedName) > -1) {
        containMatches.push({
