 ...
 */
Self.buildDataObjects = function (documents, output = `buffer`) {
  let result = [
    Self.buildLine(
      [
        `document`,
  ...
  for (let i = 0; i < documents.length; i++) {
    ...
    if (Array.isArray(_.get(doc, `dataObjects.current`)))
      for (let j = 0; j < doc.dataObjects.current.length; j++) {
        let dataObject = doc.dataObjects.current[j];
        ...
        result.push(
          Self.buildLine(
            [
              ...
              dataObject.dataType,
              dataObject.subType,
              dataObject.reuse.toString(),
              dataObject.DOI,
              dataObject.URL,
              dataObject.PID,
              dataObject.RRID,
              dataObject.version,
              dataObject.catalogNumber,
              dataObject.source,
              dataObject.suggestedEntity,
              dataObject.suggestedURL,
              dataObject.suggestedRRID,
              dataObject.associatedFigureOrTable,
              dataObject.associatedReference,
              dataObject.comments,
              sentences,
              dataObject.status,
              dataObject.actionRequired,
              documentUrl,
              pdfUrl,
              teiUrl
            ],
 ...
 * @returns {buffer} buffer
 */
Self.formatDataObjectsChangesToCSV = function (data, output = `buffer`) {
  let result = [
    Self.buildLine(
      [
        `Manuscript URL`,
 ...
 * @returns {buffer} buffer
 */
Self.formatDataObjectsHistoriesToCSV = function (data, output = `buffer`) {
  let result = [
    Self.buildLine(
      [
        `Manuscript URL`,
 ...
 * @returns {buffer} buffer
 */
Self.formatDataObjectsChangesFromReportsToCSV = function (data, output = `buffer`) {
  let result = [
    Self.buildLine(
      [
  ...
  for (let kind in data.dataObjects) {
    for (let name in data.dataObjects[kind]) {
      ...
      let line = [
        ...
        data.metadata.old.createdAt
          ? `${data.metadata.old.createdAt.getFullYear()}-${
            data.metadata.old.createdAt.getMonth() + 1
          }-${data.metadata.old.createdAt.getDate()}`
          : ``,
        data.metadata.new.createdAt
          ? `${data.metadata.new.createdAt.getFullYear()}-${
            data.metadata.new.createdAt.getMonth() + 1
          }-${data.metadata.new.createdAt.getDate()}`
          : ``,
        data.metadata.old.updatedAt
          ? `${data.metadata.old.updatedAt.getFullYear()}-${
            data.metadata.old.updatedAt.getMonth() + 1
          }-${data.metadata.old.updatedAt.getDate()}`
          : ``,
        data.metadata.new.updatedAt
          ? `${data.metadata.new.updatedAt.getFullYear()}-${
            data.metadata.new.updatedAt.getMonth() + 1
          }-${data.metadata.new.updatedAt.getDate()}`
          : ``,
        data.metadata.old.lastModifier,
        data.metadata.new.lastModifier,
        data.metadata.old.date
          ? `${data.metadata.old.date.getFullYear()}-${
            data.metadata.old.date.getMonth() + 1
          }-${data.metadata.old.date.getDate()}`
          : ``,
        data.metadata.new.date
          ? `${data.metadata.new.date.getFullYear()}-${
