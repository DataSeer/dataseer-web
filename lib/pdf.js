...
Line.prototype.addChunk = function (input) {
  ...
  let xMin = chunk.x;
  let xMax = chunk.x + chunk.w;
  let yMin = chunk.y;
  let yMax = chunk.y + chunk.h;
...
Line.prototype.isIn = function (chunk) {
  ...
  let yMin = chunk.y;
  let yMax = chunk.y + chunk.h;
  let yMiddle = chunk.y + chunk.h / 2;
  let samePage = this.p === chunk.p;
  let outY = yMiddle > this.max.y || yMiddle < this.min.y;
  let outX = xMax < this.min.x;
...
Area.prototype.addLine = function (line) {
  let xMin = line.min.x;
  let xMax = line.min.x + line.w;
  let yMin = line.min.y;
  let yMax = line.min.y + line.h;
  if (this.min.x > xMin) this.min.x = xMin;
  if (this.max.x < xMax) this.max.x = xMax;
  if (this.min.y > yMin) this.min.y = yMin;
  if (this.max.y < yMax) this.max.y = yMax;
...
Area.prototype.isNext = function (line, margin) {
  ...
  let delta = typeof margin !== `undefined` ? margin : line.h / 2;
  let samePage = this.p === line.p;
  let isTooUnder = middle - delta > this.max.y;
  let isTooUpper = middle + delta < this.min.y;
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      if (paragraphs[i].lines.length > 1)
        for (let j = 0; j < paragraphs[i].lines.length - 1; j++) {
          ...
          let next = paragraphs[i].lines[j + 1];
          let interline = Math.abs(previous.min.y + previous.h / 2 - (next.min.y + next.h / 2));
          if (typeof stats[previous.p] === `undefined`) stats[previous.p] = [interline];
          else stats[previous.p].push(interline);
        }
      else {
        let line = paragraphs[i].lines[0];
        let value = line.min.y + line.h / 2;
        if (typeof stats[line.p] === `undefined`) stats[line.p] = [value];
        else stats[line.p].push(value);
      }
    }
    let result = {};
