...
const maxDataSize = isNaN(conf.maxFileUploadSize) ? 50 * 1024 * 1024 : conf.maxFileUploadSize;
...
Self.processDataseerSentences = function (sentences, cb) {
  ...
  if (list.length <= 0) return cb(null, results);
  return async.mapLimit(
    list,
    service.limit,
    function (sentence, next) {
      if (typeof sentence !== `string`) return next();
