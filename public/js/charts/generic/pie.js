...
(function (d3) {
  ...
  for (let i = 0; i < limit; i++) {
    let _d = {
      ...
      done: isNaN(params.done[i]) || params.done[i] === `` ? i : params.done[i],
      total: isNaN(params.total[i]) || params.total[i] === `` ? limit : params.total[i],
      opacity: isNaN(params.opacity[i]) || params.opacity[i] === `` ? 0.9 : params.opacity[i],
