...
const HtmlBuilder = {
  ...
  ul: function (options = {}) {
    let result = jQuery('<ul/>').addClass(options.class).text(options.text);
    if (options.id) result.attr('id', options.id);
    if (options.value) result.attr('value', options.value);
    return result;
  },
  li: function (options = {}) {
    let result = jQuery('<li/>').addClass(options.class).text(options.text);
    if (options.id) result.attr('id', options.id);
    if (options.value) result.attr('value', options.value);
    return result;
  },
  ...
  option: function (options = {}) {
    return jQuery('<option/>').attr('value', options.value).text(options.text).prop('selected', options.selected);
  },
